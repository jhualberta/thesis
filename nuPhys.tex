...
The Lagrangian density constructed from the Dirac equation is $\mathcal{L}_D = \bar\psi(i\gamma^\mu\partial_\mu-m)\psi$, where $\bar{\psi}\equiv \psi^{\dag}\gamma^0$ is a adjoint field. The Dirac spinor field $\psi$ can be decomposed into left and right handed fields: $\psi=\psi_L+\psi_R\equiv P_L\psi+P_R\psi$. Then the Dirac Lagrangian density which satisfies the Lorentz invariance and is hermitian can be written as\cite{zee2010quantum}:
...
\end{equation}
where $y^{lep}_{ff'}$ are Yukawa coupling matrices of leptons and the $h.c.$ means hermitian conjugate. The terms are invariant under the $SU(2)_L\times U(1)_Y$ group.
...
The Higgs field obtains a vacuum expectation value (VEV) via the electro-weak symmetry breaking process. Taking the unitary gauge, $\Phi=\frac{1}{\sqrt 2}\begin{pmatrix}0\\v+h \end{pmatrix}$, where $h$ is a real scalar field. 

To diagonalize the Lagrangian \ref{yukawa2}, first taking the unitary matrices: $V^{lep}_L, V^{lep}_R,V^\nu_L$ and $V^\nu_R$ and doing biunitary transformations: $y^{lep}={V^{lep}_L}^\dagger y^{lep'}V^{lep}_R, y^{lep}_{ff'}=y^{lep}_f\delta_{ff'};y^{\nu}={V^{lep}_L}^\dagger y^{\nu'}V^{lep}, y^{\nu}_{ff'}=y^{\nu}_f\delta_{ff'}$ and also transforming the fields into: $\ell_{L,R}=({V^{lep}_{L,R}}^\dagger) \ell_{L,R}'$ and
$\nu_{L,R}=({V^\nu_{L,R}}^\dagger)\nu'_{L,R}$ where $\ell_{L,R}'=(e,\mu,\tau)_{L,R}^T$ and ${\nu}_{L,R}'=(\nu_e,\nu_\mu,\nu_{\tau})_{L,R}^T$ are in the flavor or generation space\cite{funchal2013physics,lesgourgues2013neutrino,akhmedov2014majorana}.

The product ${V_L^{lep}}^\dagger V_L^\nu\equiv U$ is actually the PMNS matrix which has the relation: $\nu_{fL} = \sum_{i=1,2,3}U_{fi}\nu_{iL}$.

Thus the Lagrangian \ref{yukawa2} becomes:

\begin{equation}
-\mathcal{L}_{Yukawa} = (\frac{v+h}{\sqrt 2})\sum_{f=e,\mu,\tau}(y_f^{lep})\overline{\ell_{fL}} \ell'_{fR}+\sum_{i=1,2,3}(y_i^\nu)\overline{\nu_{iL}}\nu_{iR}+h.c.,
\end{equation}

In this case, the Dirac mass term becomes:
\begin{equation}
-\mathcal L^D_{mass} =\sum_{f=e,\mu,\tau}\frac{v}{\sqrt{2}}(y_f^{lep})\overline{\ell}_{fL}\ell_{fR}+\sum_{i=1,2,3}\frac{v}{\sqrt{2}}(y_{i}^\nu)\overline{\nu_{iL}}\nu_{iR}+h.c.,
\end{equation}
