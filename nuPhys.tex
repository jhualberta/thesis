...
The Higgs field obtains a vacuum expectation value (VEV) via the electro-weak symmetry breaking process. Taking the unitary gauge, $\Phi=\frac{1}{\sqrt 2}\begin{pmatrix}0\\v+h \end{pmatrix}$, where $h$ is a real scalar Higgs field. 
...
-\mathcal{L}_{Yukawa} = (\frac{v+h}{\sqrt 2})\sum_{f=e,\mu,\tau}(y_f^{lep})\overline{\ell_{fL}} \ell_{fR}+\sum_{i=1,2,3}(y_i^{\nu})\overline{\nu_{iL}}\nu_{iR}+h.c.,
\end{equation}
...
\begin{equation}
-\mathcal L^D_{mass} =\sum_{\alpha=e,\mu,\tau}\frac{v}{\sqrt{2}}(y_\alpha^{lep})\overline{\ell_{\alpha L}}\ell_{\alpha R}+\sum_{i=1,2,3}\frac{v}{\sqrt{2}}(y_{i}^\nu)\overline{\nu_{iL}}\nu_{iR}+h.c.,
\end{equation}
So the charged lepton masses are given by: $m_{\ell_\alpha}=\frac{y_\alpha^{lep} v}{\sqrt 2}~(\alpha=e,\mu,\tau)$ and the neutrino masses are: $m_i=\frac{y_k^\nu v}{\sqrt 2}~(i=1,2,3)$\cite{funchal2013physics}. 
...
If neutrinos are Majorana fermions, we can take the neutrino field as $\nu=\nu_L+\nu_L^c$ without introducing extra $\nu_R$. Then the Majorana mass term is:  
\begin{equation}\label{major_mass}
\mathcal{L}^{M~L}_{\nu~mass} = -m_L\bar{\nu}\nu=-\frac{1}{2}m_L\overline{\nu_L^c}\nu_L+h.c.,
\end{equation}
where the $1/2$ factor is to avoid double counting of $\nu_L^c$ and $\overline{\nu_L}$ since $\nu_L^c=\mathcal{C}\overline{\nu_L}^T$\cite{giunti2007fundamentals}. For this Majorana mass term, If considering the $U(1)$ group transformations: $\psi\to e^{i\alpha}\psi$ and $\bar \psi \to \bar\psi e^{-i\alpha}$, where $\alpha$ is an arbitrary phase parameter, we find that the mass term in the form of $\psi\bar\psi$ is invariant in the Dirac case while it is not in the Majorana case. This means that the Majorana fields break all $U(1)$ charge, such as the electric charge and lepton number. Since the electric charge is always conserved, the particle with a Majorana mass must be electrically neutral\cite{akhmedov2014majorana}. This indicates that the lepton number for the Majorana fermion is not conserved, which is not described in the Standard Model. The neutrinoless double beta decay process discussed in the following section will come back to this point.

Also, since the term $\overline{\nu_L^c}\nu_L$ has the weak isospin $I_3=1$ and then $Y=-2$, it indicates a Higgs triplet with $Y=2$ is required, which is not included in the Standard Model. Therefore, new physics beyond the Standard Model is required for describing the Majorana mass term.

For a more general case: a hybrid mass term of Dirac and Majorana, we can write the Dirac mass term in \ref{diracLagrange} as the same form of the Majorana mass term: $\mathcal{L}_{\nu~mass}^D = -\frac{1}{2}m_D(\overline{\nu_L}\nu_R+\overline{\nu^c_R}\nu^c_L)+h.c.$. Likewise, introduce the right-handed neutrino fields to the Majorana case and then we have a Majorana mass term with right-handed component: $\mathcal{L}^{M,R}_{\nu~mass} = -m\bar{\nu}\nu=-\frac{1}{2}m_R\overline{\nu_R^c}\nu_R+h.c.$. Therefore, the hybrid mass term combining the Majorana fields (with $m_L$ for the left-handed neutrino field and $m_R$ for the right-handed field) and Dirac is\cite{akhmedov2014majorana,zuber1998physics}:
\begin{equation} 
