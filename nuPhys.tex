...
\end{equation}
where $f=e,\mu,\tau$ and $k=1,2,3$. The unitary PMNS matrix, $U_{PMNS}$, can be parametrized as\footnote{Here we ignore the Majorana CP violation phases, which are cancelled out when tackling with the flavor transformation probability.}: 
\begin{equation}
U_{PMNS} =
\begin{bmatrix}
1 &0 &0\\
...
\section{Neutrino Mass and Majorana Neutrino}
The results from the neutrino flavor transformation experiments prove that neutrinos are not massless and have finite mass. However, there is no mechanism describing the neutrino mass in the Standard Model.

For a free fermion $\psi$ following the Dirac equation $(i\gamma^\mu\partial_\mu-m)\psi=0$, the Lagrangian density is $\mathcal{L}_D = \bar\psi(i\gamma^\nu\partial_\nu-m_D)\psi$. Here $\psi$ is a Dirac spinor field and can be decomposed into left and right handed fields:
...
\begin{figure}[htbp]
	\centering	
		\includegraphics[width=3cm]{blackbox.png}
