...
The results from the neutrino flavor transformation experiments prove that neutrinos are not massless and have finite masses. However, there is no mechanism in the Standard Model to explain the neutrino masses.
...
In the Standard Model, neutrinos are the only spin-1/2 elementary particles without carrying electrical charges, which is one of the reasons why they are special. No charge-like quantum number can be used to distinguish a neutrino and an antineutrino. As a comparison, a spin-1/2 and neutral charge neutron carries a magnetic moment opposite to an antineutron\cite{akhmedov2014majorana}.
...
\{\gamma^\mu,\gamma^\nu\}=2\eta^{\mu\nu}, 
\end{equation}
where $\eta^{\mu\nu}=diag(1,-1,-1,-1)$ is the Minkowski metric. From \ref{eq:clifford} we can also get $(\gamma^0)^2=+1$, $(\gamma^i)^2=-1~(i=1,2,3)$ and $\gamma^0\gamma^{\mu\dag}\gamma^0=\gamma^\mu$. The product of four gamma matrices is defined as $\gamma^5\equiv i\gamma^0\gamma^1\gamma^2\gamma^3$, which satisfies $\{\gamma^5,\gamma^\mu\}=0$ and $(\gamma^5)^2=1$. From $\gamma^5$, we can define the left-handed and right-handed chirality projector operators: $P_L\equiv\frac{1}{2}(1-\gamma^5)$ and $P_R\equiv\frac{1}{2}(1+\gamma^5)$.
...
\begin{equation}
\hat C: \psi\to \psi^c=\mathcal{C}\bar\psi^T
\end{equation}
...
From the Black Box theorem, if the $0\nu\beta\beta$ process is observed, the Majorana nature of neutrinos is demonstrated.
...
due to their relative CP phase. With several Majorana neutrinos with different CP eigenvalues the neutrinoless diagrams discussed here may not be entirely
negligible.
...
\begin{figure}[htbp]
	...
	\caption{ A black box diagram, modified from \cite{schechter1982neutrinoless}.}
	\label{blackbox}
\end{figure}
