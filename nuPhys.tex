...
Unlike the Dirac case, for the Majorana field $\psi^M$, the $\psi^M_L$ and $\psi^M_R$ are not independent. The right-handed component can be the charge conjugate of the left-handed component (or vice versa): $\psi_R=(\psi_L)^c$\cite{akhmedov2014majorana}. Therefore Majorana field can be written as: $\psi^M=\psi_L^M+\psi_R^M=\psi^M_L+(\psi^M_L)^c=\psi^M_L+((\psi^M)^c)_R$, which immediately gives $(\psi^M)^c=\psi^M$ (Majorana condition). This result implies that the particles associated with the Majorana fields, or the Majorana fermions, are their own antiparticles and ``truly neutral''\cite{akhmedov2014majorana}. Also, the Majorana equation preserves the handedness\cite{zee2010quantum}. Since the $\psi^M_L$ and $\psi^M_R$ are not independent and can be constructed by each other, the Majorana spinor $\psi_M$ only has two degrees of freedom compared to the Dirac spinor which has four degrees of freedom, which is more economical. These facts indicate that the Majorana fermion theory is tailored for describing neutrinos. It is natural that neutrinos are of Majorana nature.

In the Standard Model, the Majorana fermions are not included. Also, the $V-A$ theory as well as the experimental observations such as the measurement of neutrino helicity\cite{goldhaber1958helicity} allow only the left-handed flavor states $\nu_f~(f=e,\mu,\tau)$ (or called ``active'' neutrinos) and right-handed flavor states $\bar\nu_f$ involving in the weak interaction. In the weak interactions theory, the leptons form the left-handed weak isospin doublets (isodoublets) as $\Psi_L = \begin{pmatrix}\psi_{\nu_f}\\ \psi_f\\ \end{pmatrix}_L$ with the weak isospin $I=\frac{1}{2}$ and its third components $I_3=\pm \frac{1}{2}$ ($I_3=+\frac{1}{2}$ for neutrino fields and $I_3=-\frac{1}{2}$ for charged lepton fields), and the right-handed isosinglets $\Psi_R = (\psi_f)_R$ with $I=0$ and $I_3=0$\cite{aitchison2012gauge}.

According to the Gell-Mann–Nishijima formula $Q=I_3+\frac{1}{2}Y$, where Q is the particle electric charge and Y is the hypercharge, $\hat Y\Psi_L = -1$ and $\hat Y\Psi_R=-2$. For charged leptons such as electrons, they acquire their masses by coupling to the Higgs fields

is also an isodoublet with weak isospin $I=\frac{1}{2}$ and $I_3=\frac{1}{2}$.
...
Using a unitary matrix $U$ to diagonalize the $\mathcal M$ so that ${U}^T\mathcal{M}\mathcal{U}=\mathcal {M}_d = diag(m_1, m_2)$, where $m_{1,2}\geq 0$ are the mass eigenstates of the eigenstates $\chi_{1,2}$ and 
