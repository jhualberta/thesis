...
\subsection{Majorana Neutrino}
In the Standard Model, neutrinos are the only fermions without carrying electrical charges, which is one of the reasons why they are special.

For a free fermion field $\psi$ following the Dirac equation $(i\gamma^\mu\partial_\mu-m)\psi=0$, the Lagrangian density is $\mathcal{L}_D = \bar\psi(i\gamma^\nu\partial_\nu-m_D)\psi$. Here $\psi$ is a Dirac spinor field and can be decomposed into left and right handed fields:
$\psi=\psi_L+\psi_R$\cite{zee2010quantum}. Then the Dirac mass term is\cite{kane2017modern}:
\begin{equation} 
\mathcal{L}_D = -m_D\bar\psi\psi= -m_D(\bar{\psi}_R\psi_L+\bar{\psi}_L\psi_R),
\end{equation} 
...
the Dirac mass term is not economical\cite{aitchison2012gauge}.
...
The interpretation of the $0\nu\beta\beta$ process is considered as exchanging light Majorana neutrinos. In this case the effective Majorana mass $\langle m_{ee}\rangle=\sum_{i=1}^{3} |U_{ei}|^2m_i~(i=1,2,3)$, where $U_{ei}$ are the elements of the neutrino mixing matrix for the flavor state $\nu_e$, and $m_i$ are the mass eigenvalues of the mass eigenstates (from (\ref{eq:mixingmatrix})). The observable quantity is the half-life:
\[
...
$D^M$ is a diagonal unitary matrix and $D^M=diag(e^{i\eta_1},e^{i\eta_2},1)$. Thu $U_{PMNS}=U^DD^M$.
