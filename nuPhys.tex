...
(** will be rephrased and shortened **)
Currently, we know that neutrinos have three leptonic flavors, and they only participate in the weak and gravitational interactions, while gravity can be neglected in the particle-scale interactions. In the weak interactions, a neutrino $\nu_\alpha$ is generated with a definite leptonic flavor, accompanied by one of the three charged leptons: electron ($e$), muon ($\mu$) or tauon ($\tau$). It is identified as an electron neutrino ($\nu_e$), a muon neutrino ($\nu_\mu$), or a tauon neutrino ($\nu_\tau$).
...
For the $\Delta m^2_{21}$ and $\theta_{12}$, the combined analysis of the measurements from the reactor experiment, the Kamioka Liquid Scintillator Antineutrino Detector(KamLAND) and SNO gave $\Delta m^2_{21} = 7.59^{+0.21}_{-0.21}\times 10^{-5}eV^2$ and $\tan^2{\theta}_{21}=0.47^{+0.06}_{-0.05}$\cite{abe2008precision}.
...
the rate of neutral current (NC) reactions,
\begin{equation}
\nu_x+d\to p+n+\nu_x,
\end{equation}

the rate of charged current (CC) reactions,
\begin{equation}
\nu_e+d\to p+p+e^-,
\end{equation}

the rate of elastic scattering (ES) reactions,
\begin{equation}
\nu_x+e^-\to \nu_x+e^-,
\end{equation}


A combined analysis of SNO data acquired from 1999 to 2006 gave the measured total flux of $^8$B solar neutrinos by $\Phi_{^8B}=5.25\pm0.16(stat.)^{+0.11}_{-0.13}(syst.)$ cm$^{-2}$s$^{-1}$
Based on a two-flavor neutrino oscillation analysis, $\Delta m^2_{21}=(5.6^+{1.9}_{-1.4})\times 10^{-5}$ eV$^2$ and $\tan^2\theta_{12}=0.427^{+0.033}_{-0.029}$.
\cite{aharmim2013combined}.

\subsection{Super-Kamiokande}
Super-K is able to lower down the energy threshold to 3.5 MeV to search for the $^8$B solar neutrinos\cite{abe2016solar}.
By utilizing the data from 2008 to 2014 (1664 live days), Super-K reported a measurement of $\Phi_{ES}=(2.308\pm0.020(stat.)^{+0.039}_{-0.040}(syst.))\times 10^6~cm^{-2}s^{-1}$.
%15 $\nu$ events/day. For the $^8$B solar neutrino flux,
\subsection{Borexino}
Borexino is a liquid scintillator neutrino detector with a target mass about 300 tonne. It is located at the Gran Sasso National Laboratory (LNGS) in central Italy, under an overburden of rock with 3800 water equivalent meter (m.w.e) to suppress the cosmogenic backgrounds. They have measured $^7$Be, pep, pp and $^8$B solar neutrino fluxes\cite{agostini2018comprehensive}. 

In 2020, Borexino reported the first observation of CNO neutrino.
...
As mentioned in Sect.~\ref{sect:VacuumOsci}, the reactor neutrino experiment KamLAND is able to study the flavor transformation in the solar section due to the long baseline of 180 kilometers from the reactors to the detector.
...
So far the solar, reactor, atmospheric and accelerator neutrino oscillation experiments can be consistently described using three active neutrino flavors, i.e. two mass splittings and three mixing angles.
...
Jinping Neutrino Experiment (Jinping) is planned to build a 4-kilotonne liquid scintillator or water-based liquid scintillator detector at China Jinping Underground Laboratory (CJPL), where the overburden of rock is 2400 m (6720 m.w.e) and the cosmic-ray muon flux there is measured as $(2.0\pm0.4)\times10^{-10}(cm^{-2}s^{-1})$\cite{beacom2017physics}, which is about 1/170 times the Borexino\cite{bellini2012cosmic}. Its fiducial mass for the $\nu-e^-$ scattering events is proposed to be 2 kiloton, which is 20 times larger than the Borexino target mass\cite{beacom2017physics}. With the advantages of low cosmogenic backgrounds and large fiducial mass target, Jinping is featured to measure pp neutrinos in the range of 0.2-0.3 MeV, 7Be pep and  CNO can for a 5-year run\cite{beacom2017physics}. 
...
\section{Neutrino Mass and Neutrinoless Double Beta Decay}\label{sect:doublebeta}
...
\begin{itemize}
	...
	\item Direct measurements of the $\beta$-decay spectrum. The KArlsruhe TRItium Neutrino (KATRIN) experiment measures the $\beta$-decay from tritium ($^3$H$\to^3$He$^+$+$e^-+\bar{\nu}_e$). In 2019, the experiment reported an upper limit of $m_\nu<1.1$ eV with 90\% C.L.. After 5 years of measurement, it is expected to reach the $m_\nu$ sensitivity down to 0.2 eV with 90\% C.L.\cite{aker2019improved}.
	 
	\item A search for the neutrinoless double beta decay ($0\nu\beta\beta$) process. This is the main physics target of the SNO+ experiment and it will be discussed in the following section.
\end{itemize}
...
Similar to the $\beta$-decay case, the $2\nu\beta\beta$ process will cause a continuous spectrum in the detector while the $0\nu\beta\beta$ process only has two electrons in the final state. These electrons take away the entire energy (the energy from the nuclear recoil is negligible here) and their energy spectrum sum up to give a distinct energy peak at the Q-value. Taking the isotope $^{130}$Te as an example, Fig.~\ref{te130energy} illustrates the shapes of the energy spectrum from the $2\nu\beta\beta$ and the $0\nu\beta\beta$ decay processes.
\begin{figure}[htbp]
	\centering	
...
For the Majorana neutrino, an effective Majorana mass $\langle m_{ee}\rangle$ is defined by:
...
\end{equation}
where the values of $U_{ei}$ are the elements of the neutrino mixing matrix for the flavor state $\nu_e$, and $m_i$ are the mass eigenvalues of the mass eigenstates, according to Enq.~\ref{eq:mixingmatrix}; $\alpha_1,\alpha_2$ are Majorana CP-violation phase factors ranging from 0 to $\pi$, and $\alpha_2$ can also be taken as $\alpha_2-\delta_{CP}$.

% For the NH, $m_{lightest}=m_1$, and $m_2=\sqrt{m_1^2+\Delta m^2_{21}}, m_3 = \sqrt{m_1^2+|\Delta m^2_{31}|}$;
% for the IH, $m_{lightest}=m_3$, $m_1=\sqrt{m_3^2+|\Delta m^2_{31}|}, m_2=\sqrt{m_3^2+|\Delta m^2_{31}|+\Delta m^2_{21}}$

the decay width and the half-life are obtained as\cite{suekane2015neutrino,zuber2020neutrino}:
\begin{equation}
\Gamma=(T^{0\nu\beta\beta}_{1/2})^{-1} = G_{PS}(Q,Z)|M_{Nuclear}|^2\langle m_{ee}\rangle^2, 
\end{equation}
where $G_{PS}(Q,Z)$ is a phase space corresponding to the effective coupling constant, which depends on the endpoint energy $Q$ and the atomic number $Z$; $|M_{Nuclear}|$ is the nuclear matrix element describing the nuclear transition and it can only be calculated theoretically from approximate methods based on many-body nuclear models, such as the Nuclear Shell Model (NSM), interacting Boson Model (IBM), etc.

A detector with high energy resolution can search for this distinct energy peak as the $0\nu\beta\beta$ signal from the certain radioactive isotopes. Diverse technologies have been developed during the past decades. 

An observation (i.e. measurement of T) would quantify the effective neutrino
mass as G and jMj2 can be calculated theoretically. The uncertainty
on a such mass measurement would additionally depend on the uncertainties
on G and jMj2. G is determined from the kinematics of this decay process
and has a relatively small uncertainty. However, recent analyses of the calculations of jMj2 and its systematics indicate that these values are likely known
...
\end{equation}
where $N_A$ is the Avogadro's number, $\alpha$ is the abundance of the isotope in the element, $M$ is the molar mass of the isotope and $t$ is the measurement time of total exposure.

There are 35 isotopes can undergo the double beta decay process, but only a few of them are suitable for the application in direct $0\nu\beta\beta$ search experiments\cite{giunti2007fundamentals}. From the experimental view, the candidate isotopes are expected to have relatively high natural abundances, high Q-values, be deployed in a large amount with low costs, not toxic to the environment, etc. However, in a realistic situation, no isotope fulfills all these properties, and trade-offs have to be made for the current experiments\cite{dolinski2019neutrinoless}. Fig.~\ref{fig:te_abundance} 



The experiments discussed below are using the $^{76}$Ge, $^{136}$Xe and $^{130}$Te isotopes.

The mainstream experiments.
%
%
%
...
\begin{itemize}
	...
	Instead of using Te crystal and cryogenic technology, the SNO+ experiment uses a novel technology by dissolving the $^{130}$Te isotope into liquid scintillator. Details will be discussed in Chapter 3.
