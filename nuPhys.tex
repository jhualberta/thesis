...
\begin{figure}[htbp]
	...
	\caption{ Solar neutrino energy spectrum ($E_\nu$ vs. flux) for the solar model BS05(OP)\cite{bahcall2005new}.}
	\label{bp05plot}
\end{figure}
...
\begin{equation}
m^2_\beta = \sum_i |U_{ek}|^2 m_i^2.
\end{equation}

\subsection{From Dirac to Majorana}
...
For fast moving particles, it's convenient to take $\gamma$ matrices in chiral or Weyl representation\footnote{In Weyl representation $\gamma_5$ is diagonal while in Dirac representation $\gamma^0$ is diagonal.} as\cite{zee2010quantum}:
\begin{equation}
...
Define the adjoint field $\bar{\psi}\equiv \psi^{\dag}\gamma^0$, the Lagrangian density constructed from the Dirac equation is $\mathcal{L}_D = \bar\psi(i\gamma^\mu\partial_\mu-m)\psi$. The Dirac spinor field $\psi$ can be decomposed into left and right handed fields: $\psi=\psi_L+\psi_R\equiv P_L\psi+P_R\psi$. Then the Dirac Lagrangian density can be written as\cite{zee2010quantum}:
\begin{equation} 
...
\end{equation} 
where the first two terms on the right hand side are kinetic energy terms, where the chiral components are separate; and the last is the Dirac mass term $\mathcal{L}^{D}_{mass}$, where the chiral components are coupled.

A particle-antiparticle conjugation operator $\hat C$ is defined as\cite{akhmedov2014majorana}:
\begin{equation}
...
\begin{equation}
\mathcal C^{-1}\psi^\mu\mathcal C=-(\gamma^\mu)^T,~~\mathcal{C}^{-1}\gamma_5\mathcal{C}=(\gamma_5)^T,~~\mathcal{C}^\dagger = \mathcal{C}^{-1}=-\mathcal{C}^*
\end{equation}

since only left-handed $\nu$ and right-handed $\bar\nu$ involve in the weak interaction in the Standard Model, 
the Dirac mass term is not economical and natural\cite{aitchison2012gauge}.
...
In the flavor transformation experiments, the mass differences, rather than the absolute mass values are measured. 

In the Standard Model, neutrinos are the only spin-1/2 elementary particles without carrying electrical charges, which is one of the reasons why they are special. No charge-like quantum number can be used to distinguish a neutrino and an antineutrino. As a comparison, a neutron is spin-1/2 and chargeless but it carries magnetic moments opposite to an antineutron\cite{akhmedov2014majorana}.

By mathematical aesthetics, Majorana found a representation of the $\gamma$-matrices as following\cite{majorana2006symmetric}:
\begin{equation}
...
$\mathcal{C}=i\gamma^2\gamma^0$, then 
$\psi^c=\mathcal{C}{\bar{\psi}}^T=(i\gamma^2\gamma^0){({(\psi^*)}^T\gamma^0)}^T=i\gamma^2\psi^*=\psi$.


In the Majorana case, the field can be taken as $\psi=\psi_L+\psi_L^c$ and thus $\psi^C=\psi$.


The Majorana mass term is\cite{kane2017modern}:  
\begin{equation}\label{major_mass}
\mathcal{L}_M = -\frac{1}{2}m_L(\bar{\nu}_R^c\nu_R+\bar{\nu}_R\nu^c_R)+h.c.,
\end{equation}

the Dirac mass term can be written as the same form: $\mathcal{L}_D =  -\frac{1}{2}m_D(\bar{\nu}_L\nu_R+\overline{\nu^c_R}\nu^c_L)+h.c.,$

A hybrid mass term combining the Majorana fields (with $m_L$ for the left-handed neutrino field and $m_R$ for the right-handed field) and Dirac is\cite{akhmedov2014majorana,zuber1998physics}:
\begin{equation} 
...
Use a real orthogonal matrix $\mathcal{O}$ to diagonalize the $\mathcal M$

so that $\mathcal{O}^T\mathcal{M}\mathcal{O}=\mathcal {M}_d = diag(m_1, m_2)$, and the mass eigenstates $\chi_{1,2}$ are correspond to $m_{1,2}$. 

$C^{-1}\gamma_\mu C=-\gamma_\mu^T$


From the mass matrix, the eigenstates are solved:
\begin{equation}
m_{1,2}\equiv m_{\mp} = \frac{1}{2}[~(m_L^M+m_R^M)\mp\sqrt{(m_L^M-m_R^M)^2+4(m^D)^2}~]\label{eq:majorana_massEigen},
\end{equation}
and with $\tan2\theta =\frac{2m_D}{m_R-m_L}$.

From (\ref{eq:majorana_massEigen}), there are 4 cases for discussion:\\
(1) If $m_L^M=m_R^M=0$ (Dirac limit), $m_{1,2}=m^D$, neutrinos are pure Dirac particles.\\
(2) If $m^D\gg m^M_{L,R}$, then $\tan2\theta=\frac{2m^D}{m^M_R-m^M_L}\gg 1$ and $\theta\simeq \pi/4$, $\frac{m_{L,R}^M}{m^D}\to 0$, $m_{1,2}=\frac{1}{2}m^D[~\frac{(m_L^M+m_R^M)}{m^D}+\sqrt{(\frac{m^M_L-m^M_R}{m^D})^2+4}~]\simeq\frac{1}{2}m^D(0+\sqrt{0+4})= m^D$, the pair of Majorana neutrinos behaves like one Dirac neutrino which is called a Pseudo-(or quasi-)Dirac Neutrino.
\\
(3) If $m^D=0$, $m_1=m^M_L, m_2=m^M_R$, neutrinos are pure Majorana particles.\\
(4) If $m_R^M\gg m^D, m_L^M$, then $\theta\simeq(\frac{m^D}{m_R^M})\ll 1$ and $\tan2\theta=\frac{2m^D}{m^M_R-m^M_L}\to 0$, we get:
\begin{equation}
\begin{aligned}
...
the Seesaw mechanism explains the smallness of neutrino masses compared to the other fermions in the Standard Model.
...
[Carnegie-Mellon Report No. COO-3066-180 (unpublished)], their contributions to the $0\nu\beta\beta$ rate cancel
due to their relative CP phase. With several Majorana neutrinos with different CP eigenvalues the neutrinoless diagrams discussed here may not be entirely negligible.
