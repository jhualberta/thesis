...
\begin{equation}
-\mathcal{L}_{Yukawa} = (\frac{v+h}{\sqrt 2})[\overline{\ell'_L} (y^{lep})'\ell'_R+\overline{N'_L} (y^\nu)'N'_R]+h.c.,
\end{equation}
...
\end{equation}
Then the charged lepton masses are given by: $m_{\ell_f}=\frac{y_f^{lep} v}{\sqrt 2}~(f=e,\mu,\tau)$ and the neutrino masses are: $m_k=\frac{y_k^\nu v}{\sqrt 2}~(k=1,2,3)$, where v is the vacuum expectation value (VEV) of the Higgs field.  
