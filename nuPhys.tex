...
and $m_L$ ($m_R$) is the mass for the left-handed (right-handed) neutrino field. The mass matrix is written as:
...
\begin{equation}
-\mathcal L=\frac{1}{2}N_L^T\mathcal{C}^{\dagger}\mathcal M N_L+h.c.=\frac{1}{2}\chi_L^T\mathcal{C}^{-1}\mathcal M_{diag} \chi_L+h.c.=\frac{1}{2}(|m_1|\bar\chi_1\chi_1+|m_2|\bar\chi_2\chi_2)
\end{equation}
...
In the case of (4), the neutrino field $\chi_2$ with the mass $m^M_R$ is right-handed and of Majorana nature. This neutrino does not take part in the Standard Model interactions and thus is sterile. If this sterile neutrino is heavy, the active neutrino $\chi_1$ will be light. Such a process is called the Seesaw Mechanism. For this mechanism, new neutral force carriers or messenger particles are added into the Standard Model by considering a dimension-5 operator $\mathcal{O}_5=\frac{\kappa}{M_X}\Psi_{iL}\Psi_{jL}\Phi\Phi$, where $\kappa$ is an unknown dimensionless coupling constant, $M_x$ is a large mass scale, $\Psi_{iL}$ is the $SU(2)_L$ lepton doublet with a specific generation $i$: $(\nu_{iL}, \ell_{iL})^T$ and the Higgs doublet $\Phi=(\phi^+,\phi^0)^T$\cite{barger2012physics}. Via symmetry breaking at the electroweak scale ($v_0=246~GeV$), the neutrino mass can be obtained as 
...
Since $\tan 2\theta\ll 1$, the mixing angle between $m^D$ and $m^M_R$ is very small. 
...
Here adding neutral heavy right-handed neutrinos is the type-I, or the conventional seesaw

a new term:
$\frac{1}{2}\nu_R^T\mathcal{C}^\dagger m_R\nu_R+h.c$ is added into the Lagrangian mass term,



\cite{xing2011neutrinos}. 
