...
\begin{figure}[htbp]
	...
	\subfigure[$klDiv$\label{fig:inputParamsTMVA:i}]{
		\begin{minipage}[b]{0.3\textwidth}
			\centering
...
As shown in Fig.~\ref{fig:TMVAfractionsBDT} and Fig.~\ref{fig:TMVAfractionsMLP}, applying the TMVA BDT and MLP selections to each of the 5000 fake datasets, the ratios of the BDT-selected event count to the true event count are $N_\mathrm{select,BDT}/N_\mathrm{sig,true}=0.9766\pm 0.05244$ for signal events, and $N_\mathrm{select,BDT}/N_\mathrm{bkg,true}=1.058\pm 0.1001$ for background events (using the histogram mean and root mean square). For the MLP case, the ratios are $N_\mathrm{select,MLP}/N_\mathrm{sig,true}=0.9802\pm0.05059$ and $N_\mathrm{select,MLP}/N_\mathrm{bkg,true}=1.050\pm0.009587$.
...
\begin{figure}[htbp]
	...
	\caption[Event count ratios from BDT outputs on 5000 fake datasets.]{Event count ratios from BDT outputs on 5000 fake datasets, with $5<E_{fit}<15$ MeV. The black histogram is the distribution of the signal fraction, and the dashed red histogram is the distribution of the background fraction.\label{fig:TMVAfractions}}
\end{figure}
