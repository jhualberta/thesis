...
Using the $4<E_{fit}<15$ MeV training dataset as an example, the distributions of these variables are shown in Fig.~\ref{fig:inputParamsTMVA}. The differences in distributions between the signal inputs (black solid lines) and background inputs (red dotted lines) can be observed.
...
The outputs of the signal/background discriminator responses ($D$) on the test sub-dataset are shown as 1D distributions in Fig.~\ref{output_separation_allE}, for the Fisher/LD, BDT and MLP methods respectively. Putting cuts on $D$ can reject the majority of backgrounds while keeping most of the signals.
\begin{figure}[htbp]
	\centering
