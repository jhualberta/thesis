...
As shown in Fig.~\ref{fig:TMVAfractions}, applying the TMVA BDT and MLP selections to each of the 5000 fake datasets, the selection ratios were found to be $N^\mathrm{MLP}_{sig}/N^\mathrm{true}_{sig}=0.9802\pm0.05059$ for signal events, and $N^\mathrm{MLP}_{bkg}/N^\mathrm{true}_{bkg}=1.050\pm0.09587$ for background events (using the histogram mean and root mean square). For the MLP case, the ratios are $N^\mathrm{BDT}_{sig}/N^\mathrm{true}_{sig}=0.9766\pm 0.05244$ and $N^\mathrm{BDT}_{bkg}/N^{\mathrm{true}}_{bkg}=1.058\pm 0.1001$. All these ratios fluctuate around 1.0, thus the TMVA selections are reasonable. To count for the systematic uncertainties from the MLP selection, an upper scale ratio 1.031 and a lower scale ratio 0.9296 were applied to the solar $\nu_e$ MC simulations for evaluating the systematics in solar $\nu_e$ flux, which will be discussed in Sect.~\ref{sect:fluxSystem}.
...
To evaluate the differences of the TMVA selection between the data and MC, the default cuts and the TMVA selections were applied to the $^{16}$N central run 107055 data and MC. Here the cut ratio for the $^{16}$N events is defined as $R=N_\mathrm{select}/N_\mathrm{default}$, where $N_\mathrm{default}$ is the number of events passed the default cuts, and $N_\mathrm{select}$ is the number of events passed the default cuts as well as the TMVA selection.
%\begin{table}[ht]
%	\small{
%	\centering
%	\caption{TMVA selection ratios between data and MC, for $^{16}$N central run 107055.  \label{table:TMVAon16N}}
%%	\begin{tabular*}{150mm}{c@{\extracolsep{\fill}}cccccc}
%%		\toprule
%%		Methods & $N_\mathrm{default}$ & $N_\mathrm{MLP}$ & $N_\mathrm{BDT}$ & $R_\mathrm{MLP}$ & $R_\mathrm{BDT}$ \\
%%		\hline
%%		data & 78671 &  65008 &  64615 & 0.8213 & 0.8263  \\
%%		MC   & 64806 & 53457 & 53947 &  0.8324 & 0.8249 \\
%%		$(R_\mathrm{data}-R_{MC})(\times 100\%)$ & -- & -- & -- & +0.14\% & -1.11\% \\ 
%%		\bottomrule
%%	\end{tabular*}
%}
%\end{table}
...
To evaluate the flux scale discussed in Sect.~\ref{sect:evaluateFlux}, the scale factors (1-0.61\%) and (1+0.14\%) were applied to the solar $\nu_e$ MC simulations for MLP and BDT, respectively. 
