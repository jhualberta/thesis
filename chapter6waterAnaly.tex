...
\begin{table}[ht]
	\centering
	\caption{Systematics for the solar $\nu_e$ analysis in the water phase. See Sect.~\ref{sect:posSystematics}, \ref{sect:dirSystematics}, \ref{sect:beta14Systematics} and \ref{sect:eneryUncertianties} for details.\label{tab:solar_uncertainties}}
	
	\begin{tabular*}{145mm}{c@{\extracolsep{\fill}}ccc}
		\toprule
		...
		x scale ($\delta_x$)& +0.07\%/-0.06\%  & $(1\pm \delta_x)x_{fit}$\\	
		y scale ($\delta_y$)& +0.02\%/-0.07\%  & $(1\pm \delta_y)y_{fit}$ \\
		z scale ($\delta_z$)& +0.08\%/-0.01\%  & $(1\pm \delta_z)z_{fit}$ \\
		$r'$ scale ($\delta_{r'}$)&  $\sqrt{\sum_{i=1}^3(\frac{\partial r'}{\partial x_i})^2({\delta^{\pm}_{x_i}})^2}$    & $(1\pm\delta^{\pm}_{r'}/100)r'$\\
		direction $\delta_\theta$  & +0.013/-0.101 & $1+(\cos\theta_\mathrm{sun}-1)/(1\pm\delta_\theta)$\\
