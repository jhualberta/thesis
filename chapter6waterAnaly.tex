...
As mentioned in Sect.~\ref{sect:kldiv}, the $klDiv$ quantity is not powerful for separating the internal $^{208}$Tl backgrounds. In this case, the internal $^{208}$Tl backgrounds give smaller values of $klDiv$, as shown in Fig.~\ref{fig:cmpExternalInternal:3}. The events triggered by the internal $^{208}$Tl backgrounds in the $5<E_{fit}<15$ MeV region could have clear Cherenkov rings in the PMT hit patterns, which makes the $\cos\theta_{Ch}$ distributions close to ones of the solar $\nu$. However, removing the $klDiv$ variable for the analysis presented in Sect.~\ref{sect:tmva} almost did not change the BDT and MLP outputs, since the two methods can put fewer weights on it.
