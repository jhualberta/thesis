...
The trained weights were applied to both the simulations and the actual data from the 92.54 live-day half-dataset.  Fig.~\ref{fig:cosThetaToSun_4to15_output} shows the results provided by the BDT and MLP selections for the $4<E<15$ MeV region. The BDT selection classifies about 68.39\% of the total data as background events while the MLP classifies about 65.08\% as background. Rather flat distributions for $\cos\theta_\mathrm{sun}$ are apparent in the plots. Since background events are predominant in the $4<E<5$ MeV region   \footnote{Applying the likelihood fit described in Sect.~\ref{sect:poisson_fit} , the analyses to follow will focus only on the $5<E<15$ MeV region to the MLP selected signal histogram of the half-dataset, it gives a signal rate of $1.224\pm0.1829$ event/(kt$\cdot$day) and a background rate of $3.986\pm0.2764$ event/(kt$\cdot$day).}.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=10cm]{cosThetaToSun_E4to15_output.pdf}
	\caption[BDT and MLP outputs for the $\cos\theta_\mathrm{sun}$ ($4<E_{fit}<15$ MeV).]{MLP and BDT outputs for $\cos\theta_\mathrm{sun}$, in the $4<E_{fit}<15$ MeV energy range. The blue shaded histogram shows the MLP-selected candidate solar $\nu_e$ events, while the solid red line shows the BDT-selected ones. The dashed blue line is for the MLP-selected backgrounds, while the dotted red line is for BDT-selected backgrounds.\label{fig:cosThetaToSun_4to15_output}}
\end{figure}
...
For the whole dataset, the TMAV methods were applied to the whole MC datasets of the solar $\nu_e$ simulations as well as the background simulations \footnote{The simulations of solar $\nu_{\mu}$ were also trained with the background simulations separately to calculate the detected $\nu_\mu$ from the oscillated solar neutrino flux mentioned in next section}. Similarly, the training subset used 70.25\% of the whole dataset (run 200004 to 205296), while the testing subset used the rest 29.75\% (run 205297 to 207718). The trained MLP and BDT weights were applied to the whole dataset.

In the region of $5<E_{fit}<15$~MeV, the outputs from the MLP and BDT were fitted to obtain $N_{sig}$ and $N_{bkg}$. Fig.~\ref{fig:wholeDataset_poissonFit:a} and \ref{fig:wholeDataset_poissonFit:b} show their results respectively. For comparison, the results from applying the default cuts are shown in Fig.~\ref{fig:wholeDataset_poissonFit:c}. The fit results are summarized in Table~\ref{table:wholedata_output}. 
...
\begin{table}[ht]
	...
	\begin{tabular*}{150mm}{c@{\extracolsep{\fill}}cccccc}
		...
		Default & 119.3 $\pm$ 11.96 & 42.67 $\pm$ 8.15 &  0.90 $\pm$ 0.09 & 0.32 $\pm$ 0.06 & 0.19\\
		\bottomrule
	\end{tabular*}
}
\end{table}
...
\begin{equation}
\Phi^{total}(\mathrm{^8 B})=f^{tot}_s\cdot \Phi^{total}_{MC}=(4.62 \pm 0.447(stat.))\times 10^6~\mathrm{cm^{-2}s^{-1}}\; ,
\end{equation}

and the estimated elastic scattering flux is:
\begin{equation}
\Phi^{ES}=f^{ES}_s\cdot \Phi^{total}_{MC}=(2.10\pm 0.204(stat.))\times 10^6~\mathrm{cm^{-2}s^{-1}}\; .
\end{equation}
...
Using the quadrature sum ($\sigma^2_{tot}=\sum_i \sigma^2_i$) which assumes that all the systematic variables are independent, the total systematic uncertainty of the total flux fraction $f^{tot}_s$ is expected to be: $({f^{tot}_s})^{+0.2999}_{-0.1370}$. Using the same procedure to evaluate the systematics of the elastic scattering flux fraction $f^{ES}_s$ gives $({f^{ES}_s})^{+0.1695}_{-0.008324}$.
...
\begin{equation}
{\Phi^{total}_{fit}(\mathrm{^8 B})=f_s\cdot \Phi^{total}_{MC}=(4.62\pm 0.447(stat.)^{+0.126}_{-0.105}(syst.))\times 10^6~\mathrm{cm^{-2}s^{-1}}}\; ,
\end{equation}
while the ES flux is:
\begin{equation}
\Phi_{\mathrm{ES}}=f_s\cdot \Phi^{total}_{MC}=(2.10\pm 0.204(stat.)^{+0.0528}_{-0.0455} (syst.))\times 10^6~\mathrm{cm^{-2}s^{-1}}\; .
\end{equation}
