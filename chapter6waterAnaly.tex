...
As shown in Fig.~\ref{fig:TMVAfractions}, applying the TMVA BDT selection to each of 5000 fake datasets, the ratios of the BDT-selected event count to the true event count are $(91.26\pm 4.77)\%$ for signal events, and $(117.1\pm 10.46)\%$ for background events (using the histogram mean and root mean square). 
...
\begin{figure}[htbp]
	...
	\subfigure[$klDiv$\label{fig:cmpExternalInternal:3}]{
		\begin{minipage}[b]{0.4\textwidth}
			\centering
			\includegraphics[width=5cm]{klDiv_cmpExternalInternal.pdf}
		\end{minipage}
	}
	...
	\caption[The distributions of six variables from the simulated internal and external $^{208}$Tl backgrounds.]{The distributions of six variables from the simulated internal and external $^{208}$Tl backgrounds, with $5<E_{fit}<15$ MeV. The solar $\nu_e$ signal distributions are also compared. Here the ``external backgrounds'' include the AV and external water backgrounds. The distributions of the internal (external) backgrounds are shown in solid (dashed) red lines, while the signals are shown in solid black lines. The distributions are normalized to their integrals.\label{fig:cmpExternalInternal}}
\end{figure}
...
As mentioned in Sect.~\ref{sect:kldiv}, the $klDiv$ quantity is not powerful for separating the internal $^{208}$Tl backgrounds. In this case, the internal $^{208}$Tl backgrounds give smaller values of $klDiv$, as shown in Fig.~\ref{fig:cmpExternalInternal:3}. The events triggered by the internal $^{208}$Tl backgrounds in the $5<E_{fit}<15$ MeV region could have clear Cherenkov PMT hit patterns, which makes the $\cos\theta_{Ch}$ distribution close to the solar $\nu$ distribution. However, removing the $klDiv$ variable for the analysis presented in Sect.~\ref{sect:tmva} almost did not change the BDT and MLP outputs, since the two methods put fewer weights on it.
...
\begin{equation*}
\vec{X'}_{fit}=((1+ \Delta x^\pm)x_{fit},(1\pm \Delta y^\pm)y_{fit},(1\pm \Delta z^\pm)z_{fit})\; .
\end{equation*}
