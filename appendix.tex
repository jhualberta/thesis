...
\section{Levenberg-Marquardt Method for Minimization}\footnote{This section refers to \cite{press2007numerical}.}\label{appendix:MRQ}
...
The MRQ method is the core algorithm in the \texttt{MP fitter} framework for fitting the results. A few parameters can be optimized in practice.
...
the parameters used by the \texttt{MP fitter} for different phases are summarized in Table.~\ref{table:MRQ_params}. 

\section{Calculations of the Derivatives}
...
where the derivative $\frac{dL_{\mathrm{Ch}}}{dt_{\mathrm{Ch}}}$ can be calculated numerically on the binned timing $pdf$ histogram.

\section{Likelihood Surfaces}
