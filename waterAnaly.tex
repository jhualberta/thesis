...
To create the fake datasets, I randomly and uniformly selected $N^r_{sig}$ ($N^r_{bkg}$) events after the cuts from the solar $\nu_e$ (merged backgrounds) MC simulations. For each randomly selected event, the values of $E_{fit}$ and $\cos\theta_{sun}$ were recorded. Each dataset was fitted with the maximum likelihood function described in \ref{poisson_fit}. Fig.~\ref{ensemble_test} shows an example of the fitted results.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=10cm]{ensemble_fitExample.pdf}
	\caption{An example of the randomly generated $\cos\theta_{sun}$ fitted with $(N_{sig},N_{bkg})$. The black dots are data points and the red line is the fitted results. For $N^{rand}_s = 73$ and $N^{rand}_b=44$, the fitted results give $N_{sig} = 73.42\pm9.42$ and $N_{bkg} = 43.58 \pm 7.73$, with a $\chi^2/ndf = 60.19/40 = 1.50$.}
	\label{ensemble_test}
\end{figure} 

The fit pull and the fit bias were defined by \cite{leta}:
\begin{equation}
bias=\frac{N_{fit}-N^{rand}_s}{N_{fit}},
\end{equation}

\begin{equation}
pull=\frac{N_{fit}-N^{rand}_s}{\sigma_{fit}},
\end{equation}
...
\begin{table}[ht]
	...
	\begin{tabular*}{160mm}{c@{\extracolsep{\fill}}ccccccc}
		\toprule
		Methods & $N_{sig}$ & $N_{bkg}$ & $R_{sig}$ & $R_{bkg}$ & $-2\ln L_{best}$ & $\chi^2/ndf$ \\
		\hline
		BDT &$120.78\pm11.93$ & $35.21\pm7.53$ & $0.91\pm0.09$ & $0.27\pm0.06$ & 54.96 &62.13/40\\
		MLP &$120.89\pm12.00$ & $39.14\pm7.89$ & $0.91\pm0.09$  & $0.30\pm 0.06$  & 49.52 & 45.52/30 \\
		%default cuts& \\ 
		\bottomrule
	\end{tabular*}
\end{table}
...
\begin{table}[ht]
	...
	\begin{tabular*}{80mm}{c@{\extracolsep{\fill}}cc}
		...
		XYZ scale &  \\
		XYZ shifts&  \\
	     $\delta_\theta$ (positive/negative) & +0.097/-0.008\\
	     $E_{scale}$ &  & \\
	     $E_{resol}$ &  & \\
	     $\beta_{14}$ scale &  & \\
		\bottomrule
...
energy scale
the reconstructed MC energies were scaled up and down
the expected number of signal events, the uncertainty on the expected number of events was obtained to be $^{+0.05}_{-0.06}$.
...
A remapping of $\cos\theta_{sun}$ by using the equation \ref{remapTheta} was applied on the spectrum for evaluating the systematics.
