...
As mentioned in Chapter 3, Sect.~\ref{sect:overview}, since the depth of the SNO+, the cosmogenic backgrounds induced by the cosmic muons are very few. Most backgrounds come from radioactive isotopes inside or around the detector, mostly from the $\beta$-decays of $^{214}$Bi and $^{208}$Tl. MC simulations were produced. 

The instrumental backgrounds can be removed by the data-cleaning approaches. Here the data-cleaning masks were applied.
%Table. is a summary of the $\beta$-decays from these two isotopes. 
...
For the energy scale, the reconstructed MC energies
$T'_e = (1\pm\delta_E)T_e$.

 were scaled up and down the expected number of signal events, the uncertainty on the expected number of events was obtained to be $^{+0.05}_{-0.06}$.

energy resolution, smear the reconstructed MC energies by a normalized Gaussian distribution:
$E'_{fit}=Gaus(energy,\sigma)$, 
$\sigma^2 = \sigma^2_E[(1+\delta_\sigma)^2]-1$, and $\sigma_E=\sqrt{E_{fit}}$
...
Applying the same systematics in Table.~\ref{table:ReconSystematics} on the MC ES $PDF$, and then do the same flux fit to the data, it finds that the position shift smearing is negligible while the direction and energy smearing give the largest uncertainties. The other systematic results are listed in Table.~\ref{table:smearingResults}.
...
\begin{table}[ht]
	...
	\begin{tabular*}{80mm}{c@{\extracolsep{\fill}}cc}
		\toprule
		Systematics & systematics from fitting $f_s$\\
		\hline
		\\
		x shift & ${f_s}^{+0.000003}_{-0.000045}$\\	
		y shift & ${f_s}^{-0.00005}_{-0.0001386}$\\
		z shift & ${f_s}^{+0.0001}_{-0.000138}$\\
		position scale & ${f_s}^{+0.001324}_{-0.00115}$\\\	
		$\delta_\theta$  &${f_s}^{+0.312188}_{-0.014131}$\\
		$E_{scale}$ & ${f_s}^{+0.030241}_{-0.027303}$\\
		$E_{resol}$ & ${f_s}^{+0.00026}_{-0.00026}$ \\
		$\beta_{14}$ shifts & ${f_s}^{+0.000178}_{-0.000558}$\\
		\bottomrule
	\end{tabular*}
\end{table}

Using the quadrature calculations, the total systematics is expected to be: ${f_s}^{+0.47}_{-0.22}$. Adding this systematics, the total flux is:
\begin{equation}
\Phi^{total}_{fit}(^8 B)=f_s\cdot \Phi^{total}_{MC}=(4.48\pm 0.44(stats.)^{+2.54}_{-1.23}(syst.))\times 10^6~cm^{-2}s^{-1}.
\end{equation}
...
By using the same procedure to evaluate the systematics, it gives ${f_s}^{+0.172}_{0.145}$.

Thus, for the nominal $^8$B solar neutrino flux, the ES flux is evaluated as:
\begin{equation}
\Phi_{ES}=f_s\cdot \Phi^{total}_{MC}=2.01\pm 0.20(stat.)^{+0.94}_{-0.79} (syst.)\times 10^6~cm^{-2}s^{-1}.
\end{equation}
