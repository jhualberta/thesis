...
Fig.~\ref{TMVA_bkgs_1} shows the energy spectrum of simulated internal events with their fitted positions inside the 5.5-m fiducial volume, i.e., with a cut of $R'_{fit}<5.5~m$, where $R'_{fit}$ is the magnitude of the reconstructed event position $\vec{X}_{fit}$ after the AV coordinate correction: $R'_{fit}=\sqrt{x^2_{fit}+y^2_{fit}+(z_{fit}-108)^2}$. The 108 mm offset in $z$ was discussed in Chapter 3 and Chapter 4. 
...
The main analysis is focused on the [5,15] MeV energy region.

default cuts: $-11<Z_{factor}<1$, $scaleLogL>10$, $0<G_{test}t<1.9$, $U_{test}<0.95$, $ITR>0.55$, $-0.12<\beta_{14}<0.95$.

\begin{table}[ht]
	\centering
	\caption{Number of candidate events.}
	\label{table:eventNumbers}
	\begin{tabular*}{150mm}{c@{\extracolsep{\fill}}cccc}
		\toprule
		Method & events cut off & fitted signals & fitted backgrounds \\
		\midrule
		BDT &   &  1\\
		MLP &   & \\
		default cut & \\
...
To check the uncertainty of the Poisson fit, for the $FV<5500~mm$, 5000 fake datasets were generated.  
