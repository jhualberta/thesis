...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=5cm]{n16_decay.png}
	\caption{$^{16}$N main decay scheme, modified from \cite{dragowsky200216n}.}
	\label{n16decay}
\end{figure}


Fig.~\ref{n16pic} shows the geometry of the $^{16}$N source chamber.
The chamber is a stainless steel cylinder contains 


 The chamber was designed to confine the electrons from $^{16}$N decay within the chamber and let them be detected by the PMT inside; 
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=9cm]{n16geom.png}
	\caption{$^{16}$N calibration source. Left: a diagram , ; Middle: source geometry implemented in RAT, modified from \cite{n16geom_zach}; a picture of the $^{16}N$ source, taken from \cite{n16pic}.}
	\label{n16pic}
\end{figure}
