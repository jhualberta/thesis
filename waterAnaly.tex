...
It finds that the position and $\beta_{14}$ smearing are negligible while the direction and energy smearing give more obvious uncertainties. 
\begin{table}[ht]
	\centering
...
Using the quadrature sum ($\sigma^2_{tot}=\sum_i \sigma^2_i$) which assumes that all the systematic variables are independent, the total systematics is expected to be: ${f_s}^{+0.0240}_{-0.0142}$.
...
With the systematic uncertainties obtained from the previous section, the total flux is:
\begin{equation}
