...
\begin{equation}
v_\mathrm{gr}=\frac{2 r_{PSUP}}{(T_1+T_2)}.
\end{equation}

The final estimate for $v_\mathrm{gr}$ was calculated by averaging the $v_\mathrm{gr}$ values obtained from all the events and triggered PMTs in both run-100934 and run-107055. It was found that $v_\mathrm{gr}=c/n_{water,eff}=216.478$ mm/ns, where $n_{water,eff}=1.38486$.

The SNO+ collaboration used a more complicated approach to measure actual group velocities in the SNO+ water detector by analyzing a set of laserball calibration runs \cite{groupVmeasure,anderson2021optical}. That analysis can give a more accurate value for $v_\mathrm{gr}$, but it it was not applied here.
