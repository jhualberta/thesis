...
The SkyShine classifier aims to discriminate "sky shine" events from other backgrounds by looking at the ratio of hit counts in a middle z range and a low z range. It can also look at neck and high-z OWL PMT hits. The classifier's behavior for partial fill is studied using simulation.\cite{skyshine}
...
\subsubsection{Sky-shine vs Reconstruction Positions}
Test on the simulations of $^{208}$Tl decay events.


The plots shows events with small values of Sky-shine calculations can be tagged as neck events.


\subsubsection{Optimize Sky-shine Cuts}
Apply different cuts of Sky-shine classifier values ($skyShine$),

plot xfit - xmc when	skyShine$>1,	>1.01,	...	,>1.99,	>2.0$ with a 0.01 step.
...
Compare to the numbe of MC events inside the scintillator $sqrt(x2+y2+(z108)2)<6005~mm \& z-108<6000~mm$, number of MC events/number fitted events
after cut:
...
The Hough transformation algorithm was suggested by Eugene W. Beier for the reconstruction of event direction in the SNO+ scintillator phase.\cite{houghTransform}
