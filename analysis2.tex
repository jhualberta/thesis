...
MC simulations of the $^{208}$Tl decay events were produced in a partial-fill geometry with the water level set 
at 4435~mm (in AV coordination) and 2 g/L PPO concentration. The $^{208}$Tl decay events were distributed uniformly in the scintillator region. RAT version 6.16.9 was used.
...
\begin{figure}
	...
	\caption{MC positions (left) and fitted positions (right) of $^{208}$Tl simulations.}
	\label{Tl208simu}
\end{figure}


Recall that the neck of the detector is 8-m tall, in PSUP coordination, the bottom of the neck is at $z_{neck}=6108~mm$ while the top of the neck is about $14000~mm$, which means that only a small volume of the neck cylinder is inside the PSUP sphere. In the MC simulations, events are distributed in the AV as well as the whole region of the neck.
