...
\begin{itemize}
	...
	For each event, this classifier loops through the triggered PMTs (hits), calculates $t_{res}$ for each, and determines the fraction of hit PMTs for which $t_{res}$ falls within an optimized ``prompt time window''. In the water phase, the time window was $[-2.5,5.0]$~ns. If the ITR ratio is too low for an event, a large proportion of the hit PMTs was not triggered prompt light, suggesting that the event probably did not originate from Cherenkov light; it could be instrumental noise or caused by a large amount of light reflecting off detector components (called ``late light''). 
...
Fig.~\ref{fig:EvsBeta14} shows the relations between the reconstructed energies ($E_{fit}$) and $\beta_{14}$ values from $^{16}$N central run 107055, comparing the \texttt{MPW} and \texttt{RAT} results from the data and MC, respectively. 
...
\begin{table}[ht]
	...
	\begin{tabular*}{120mm}{c@{\extracolsep{\fill}}cccc}
		...
		\hline 
		$x$ shift & 0.50$\pm$5.98 & +6.48/-5.98 & $x+\Delta x$\\	
		$y$ shift & 2.02$\pm$4.11 & +6.13/-4.11 & $y+\Delta y$\\
		$z$ shift & 1.89$\pm$4.82 & +6.71/-4.82 & $z+\Delta z$\\
		\bottomrule
	\end{tabular*}
\end{table}
...
\begin{figure}
	...
	\caption{Vertex shifts along $x, y, z$ axes and fitted with linear functions: $shift = p_0+ p_1\cdot x_i$, $(x_i=x,y,z)$.\label{fig:vertexScale}}
\end{figure}
...
\begin{table}[ht]
	...
	\begin{tabular*}{130mm}{c@{\extracolsep{\fill}}cccc}
		...
		\hline 
		$x$ scale $(\delta_x)$ &  0.005$\pm$0.021 & 0.048 & +0.07/-0.06\\	
		$y$ scale $(\delta_y)$ & -0.027$\pm$0.018 & 0.030&  +0.02/-0.07\\
		$z$ scale $(\delta_z)$ & 0.032$\pm$0.020 & 0.027&  +0.08/-0.01\\
		\bottomrule
	\end{tabular*}
\end{table}
