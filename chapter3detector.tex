...
Each channel recorded the signals from each PMT individually. With the two-PMT setup, I applied coincidence time mode measurements. In the coincidence mode, a coincidence time window between two channels was set to 48 ns, based on checking the recorded waveforms from two channels. For a certain event, the \texttt{CoMPASS} software compares the event time difference between two channels and only records it if the event time difference is less than 48 ns. A smaller window of 10 ns was further applied for analysis.
...
First, from the energy spectrum, the single $p.e.$ peak was fitted with an asymmetric Gaussian function ($f_{asym}$ in Eqn.~\ref{eq:asyGaus}), as shown in Fig.~\ref{fitSinglePE} \footnote{This fitting is suggested by Ref.~\cite{von2016measurement}, which allows for non-zero skewness due to contributions from background peaks.}. The mean value of the asymmetric Gaussian ($p_0$) represents the ADC channel number corresponding to the single $p.e.$ peak for the weighting. 
...
\begin{figure}[htbp]
	...
	\caption[The single $p.e.$ peak fitted with an asymmetric Gaussian function.]{The single $p.e.$ peak is fitted with an asymmetric Gaussian function ($f_{asym}$) to obtain the ADC channel for weighting. Here the parameters $(p_0,p_1,p_2,p_3)$ represent $(\mu, \sigma, \alpha, c)$ respectively in Eqn.~\ref{eq:asyGaus}. The mean value of $p_0$ (or $\mu$) is used as the ADC channel number relative to a single $p.e.$ peak.}
	\label{fitSinglePE}
\end{figure}

To define the multiple $p.e.$ region for the counting, the spectrum projected on each channel with and without coincidence cut were compared to define a threshold of the ADC channel for counting. By integrating from this threshold, the total numbers of events between the two spectra were close to each other. From two channels, I got two thresholds and then define a box cut in the 2D coincidence plot. I weighed the events in the box to obtain the total number of photons.
\begin{figure}[htbp]
	\centering	
...
Two dimensional (2D) spectrum of LAB-PPO with coincidence cut is shown in Fig.~\ref{2DboxCut}. A box cut is defined for multiple $p.e.$ counting. 
