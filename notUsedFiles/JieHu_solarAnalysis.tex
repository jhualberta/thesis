...
\title{An Alternative LowBG Solar Neutrino Analysis\\ by Using MultiPath Water Fitter}
...
Other packages developed for high energy particle physics, such as \texttt{StatPatternRecognition (SPR)}\cite{sprWebsite}, or more general tools for deep learning, such as \texttt{Keras}, \texttt{PyTorch} and \texttt{TensorFlow},
can also be considered as an alternative tool or as a reference for results comparisons. 

\subsection{Likelihood Fits for Solar Neutrino Candidate Events}\label{sect:solarFit}
...
A remapping of $\cos\theta_{sun}$ by using the equation \ref{remapTheta} was applied on the spectrum for evaluating the systematics. Its impact is $^{+0.3}_{-0.2}$ events.
...
In addition to the fitting of the whole energy region of $[5,15] MeV$, different energy bins with 1 MeV step were also fitted. Fig.~\ref{fig:E5to6} to Fig.~\ref{fig:E10to15} show the results.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=10cm]{1.png}
	\caption{Fit with $5<E<6$ MeV.}
	\label{fig:E5to6}
\end{figure}
...
\begin{figure}[!htb]
	...
	\caption{Fit with $6<E<7$ MeV.}
	\label{fig:E6to7}
\end{figure}
\begin{figure}[!htb]
	\centering
	\includegraphics[width=10cm]{3.png}
	\caption{Fit with $7<E<8$ MeV.}
	\label{fig:E7to8}
\end{figure}
\begin{figure}[!htb]
	\centering
	\includegraphics[width=10cm]{4.png}
	\caption{Fit with $8<E<9$ MeV.}
	\label{fig:E8to9}
\end{figure}
...
\begin{table}[ht]
	...
	\begin{tabular*}{120mm}{c@{\extracolsep{\fill}}cccc}
		...
		E (MeV)& $N_{expected}(\nu_e)$ & $N_{expected}(\nu_\mu)$ & $N_{fit,sig}$\\
		\hline 
		$[5,15]$ & 336 & 53 & 119.40$\pm$11.84\\
		$[5,6]$ & 68 & 11 & 26.07$\pm$5.72\\
		$[6,7]$ & 91 & 14 & 36.52$\pm$6.70\\
		$[7,8]$ & 68 & 11 & 20.87$\pm$4.95\\
		$[8,9]$ & 47 & 7 & 16.24$\pm$4.30\\
		$[9,10]$ & 30 & 5 & 11.50$\pm$3.56\\
		$[10,15]$ & 32 & 5 & 9.09$\pm$3.06\\
		\bottomrule
	\end{tabular*}
\end{table}
with oscillation weighting: 25,32,23,15,9,9,

For a nominal $^8B$ solar neutrino flux $\Phi_{MC}=5.46\times 10^6~cm^{-2}s^{-1}$ (was $5.69$), the flux is estimated to be $\Phi_{ES}=\Phi_{MC}\cdot N_{fit,sig}/336=1.94\pm0.19 (stat.)\times 10^6~cm^{-2}s^{-1}$, compared to the Super-K measurement: 
\begin{equation}
\Phi_{ES}=(2.345\pm0.039)\times 10^6~cm^{-2}s^{-1},
\end{equation}
and the SNO+ 2018 published results:
\begin{equation} 
\Phi_{ES}=2.53^{+0.31}_{-0.28}(stat.)^{+0.13}_{-0.10}(syst.)\times 10^6~cm^{-2}s^{-1}.
\end{equation}
