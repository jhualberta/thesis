...
To tackle with the $^{16}$N calibration data and simulations, an FECD tag cut was first applied during the data processing to only save the events when the source trigger fired. Besides this cut, several high level cuts based on classifiers were used.  
...
\subsubsection{Position Uncertainties}

\subsubsection{Position Scale Uncertainties}
The position scale uncertainty $\delta_r$ is defined by:
...
\begin{table}[ht]
	\centering
	\caption{The MPW fitter vertex shift systematics in x, y and z axes.}
	\vspace{3mm}
	\label{vertex shifts}
...
\begin{figure}[!htb]\label{direction_scan}
	...
	\caption{Relative differences of $\beta_M$ and $\beta_S$ as a function of the $^{16}$N source position. For simplicity, the corner scans are not shown in this figure. The red squares show results from the x-scan runs; green circles show the y-scan runs and the blue triangles show the z-scan runs.}
...
By simulating electron events with different energies,
...
A default fit range of [3.0, 8.8]~MeV was suggested by \cite{waterunidoc} to remove poorly reconstructed events due to the trigger inefficiency.



Fig.~\ref{fittedEnergyResol} shows the reconstructed energy spectrum.
