...
A quantity of scaled logL ($scaleLogL$) is used as the figure of merit for the position reconstruction ($posFoM$): $scaleLogL = logL/NHit{selected}$. This quantity utilizes the best log-likelihood returned by the MultiPath fitter for a successfully reconstructed event vertex, and then it is scaled by the ``selected'' $NHit$, i.e., the number of the PMTs actually used by the fitter for reconstructing the event vertex, after the PMT selections mentioned in \ref{sect:PMTselector}. Fig.~\ref{posBiasVsFOM} shows the $scaleLogL$ and the biases between .


\begin{figure}
	\subfigure[MC.]{\label{fig:biasFom1}\includegraphics[width=9cm]{posBiasVsFOM_mc.png}}
	\subfigure[Data.]{\label{fig:biasFom2}\includegraphics[width=9cm]{posBiasVsFOM_data.png}}
...
\begin{figure}
	...
	\caption{Fit direction resolutions ($|\vec{X}_{fit}-\vec{X}_{MC}|$) vs $scaleLogL$ for the $^{16}$N central run-107055.}
	\label{angularResolScan}
\end{figure}

A $scaleLogL>10$ cut on the MC (data) removed
() events 
with large biases of $d(\vec{X}_{fit},\vec{X}_{src})>6000~mm$;
...
Here the ``true'' direction of an event is defined as the direction pointing from the source position to the reconstructed position: $\vec{u}_{true} = \vec{X}_{fit}-\vec{X}_{source}$. The angle $\theta$ is the displacement between the ``true'' and the reconstructed directions and $\cos\theta=\vec{u}_{true}\cdot \vec{u}_{fit}$.

To describe the biases between the reconstructed direction and the true particle direction of the event, an angular resolution function was adopted by SNO\cite{boulay2004direct} and it is defined as a combination of two exponential components:
\begin{equation}
...
To extract the direction resolutions, a few cuts relating to the position and energy reconstruction were applied. These cuts were suggested by the SNO+ analysis to mainly remove the instrumental backgrounds ($ITR>0.55$, $-0.12<\beta_{14}<0.95$) and poor reconstructions for the events close to the source container\cite{waterunidoc}.

A distance cut of $1000~mm<|\vec{X}_{fit}-\vec{X}_{src,manip}|<2300~m$ was applied. To further suppress the effects from the tails caused by mis-reconstructions, more cuts were applied: 

For internal scans, a radius cut $R'<5850~mm$ was also applied. This radial cut was not applied on the external and neck scans.

A fit range of [0.3,1] was used. For 
...
\begin{table}[ht]
	\begin{tabular}{cccccccc}%{|p{2.2cm}|p{1.8cm}|p{2cm}|p{2cm}|p{1.8cm}|p{1.1cm}|p{1.1cm}|p{1.1cm}| }
		...
		\hline
MPW data & 3.888$\pm$0.246 & 17.96$\pm$1.10 & 0.550$\pm$0.030 & 80.69/66 & 0.974 & 0.740 & 0.384\\
MPW MC & 4.487$\pm$0.223 & 22.23$\pm$1.41 & 0.590$\pm$0.027 & 74.01/66 & 0.962 & 0.759 & 0.450\\		
		\bottomrule
	\end{tabular}
...
To improve the fit performance, for all the internal scan data (MC simulations), the angular resolution function was first fitted with 3 free parameters: $\alpha_M$, $\beta_S$, $\beta_M$. An averaged value of the fitted $\alpha_M$ for all the data (simulations) was calculated as 0.6137 for data (0.5834 for MC).


$R'<5850~mm$
$E_{fit}>3.5~MeV$, the distance between the event reconstructed position to the source position $900~mm$ and less than $2300~mm$


Fit range [0.3, 1]
fit range started at 0.3, if the events were too few, the 
range was enlarged by a 0.1 step to ensure more than 5000 events were fitted.
-0.5

and then was set as a fixed parameter. With the fixed value of $\alpha_M$, the angular resolution function
was refit with $\beta_S$ and $\beta_M$.
