...
To properly process and record information from PMTs, each DB tackling with 8 channels has three types of custom-made integrated circuits to implement three functions:
\begin{itemize} 
...
\item[$\bullet$] Two eight-channel charge integrator (SNOINT) chips integrate the PMT pulse over time interval to obtain PMT charge information. There are three different time windows for the integral, resulting in three types of charge values: QHS　(Charge High gain Short integration time), by integrating the pulse over 60 ns; QHL  (Charge High gain Long integration time), by integrating the pulse over 400 ns; and the QLX (Charge Low gain), by integrating the low gain PMT pulse and integrate over either time window.
\end{itemize}  

First, the discriminator checks the input pulse with the predefined threshold voltages. If the PMT pulse crosses the channel threshold, a discrete square trigger pulse is generated. The trigger pulse has different time widths for different trigger types, including a 93 ns long square pulse (N100) and a 48 ns square pulse (N20). At the same time, CMOS chips produces a time-to-amplitude (TAC) voltage.  as the PMT hit time. 

Each trigger pulse has three different threshold values of NHits. During the water phase, N100Med  


N100Lo, N100Med, N100Hi are triggers based on the N100 pulses, where each has a different threshold value set.

In each crate, the trigger pulses from the 16 FECs are sent to and summed up by a Crate Trigger Card (CTC). 19 CTCs are connected to 7 Master Trigger Cards/Analogue (MTC/A+) (``+'' refers to an upgrade to the old SNO system), which are installed in an individual rack. The MTC/A+ sums the trigger pulses from all crates and checks whether the sum is above threshold. 

The digital master trigger system (MTC/D) checks 
...
the number of hit PMTs in the time window, 





the TAC is stopped when the GT is issued;if there is no GT, the TAC resets and the PMT hit is not recorded as an event.

 This global trigger defines an event within the detector and results in all the associated information being saved. Multiple different trigger thresholds can be set.  So it is possible, and likely, that an event within the detector passes several of these trigger thresholds.
...
event data: 
trigger types
trigger ID number (GTID)
MTC/D clock time
GPS clock time
CAEN digitized trigger pulses
hit information: 
the integrated charges and time from an event are stored in each channel. The values of the TAC, QHS, QHL, QLX (i.e., the time and charge information of triggered PMTs) and status flags.

Once the global trigger signal is issued, the integrated charges and time from an event are stored in each channel. The hit information are read out by a Crate Controller Card (XL3) and sent to the Data Acquisition System (DAQ) and event builder, 
