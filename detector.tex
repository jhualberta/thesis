...
Two kinds of calibration sources are used by SNO+: optical sources to calibrate the detector optical properties and radioactive sources to calibrate the detector energy response. 

The optical calibration sources are used to calibrate the PMT response and to measure the optical properties of the detector media. The sources mainly include a light diffusing sphere (called ``laserball", LB), as well as the Embedded LED/Laser Light Injection Entity (ELLIE).


: energy scale, resolution, systematic uncertainties
...
Six underwater cameras were installed on the PSUP 
...
\subsubsection{ELLIE}
Due to the requirements of very low radioactive background levels for the SNO+, frequent deployments of calibration source inside the AV are needed to be avoided. To conduct weekly calibrations of PMTs, the ELLIE system was developed by SNO+. It uses light injection devices which are outside the AV and mounted on the PSUP at fixed positions.

The ELLIE system consists of three modules, which are: 
\begin{itemize}  
	\item[$\bullet$] Timing module (TELLIE)
	...
	\item[$\bullet$] Attenuation module (AMELLIE)
	optical attenuation in the detector over time
	
	\item[$\bullet$]   scattering of the detector media 	(SMELLIE) 
\end{itemize}
...
This source is inherit from the SNO experiment\cite{dragowsky1999sudbury,dragowsky200216n,hamer2001energy}, 
