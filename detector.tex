...
As mentioned in \ref{overview}, the PMTs as photon sensors are the basic detection elements for the SNO+ detector. 
...
The anode pulse produced from the PMT travels along 35 m-long RG59/U type coaxial cable (with a resistance of 75 $\Omega$) to the front-end electronics which are set up on the deck above the detector. The coaxial cable also carries the high-voltage\cite{boger2000sudbury}. To control, manage and take data from over 9000 PMTs in the SNO+ detector, the coaxial cables connected to eight PMTs are grouped into one bundle and the bundle is connected to a Paddle Card. Four Paddle Cards are linked to a PMT Interface Card (PMTIC). The PMTIC supplies high voltages and receives the signals of up to 32 PMTs. The 32 channels in PMTIC are connected to a Front End Card (FEC) that processes, digitizes and stores PMT signals. Each FEC has 4 Daughter Boards (DB) and each DB handles 8 channels. 16 PMTICs and FECs are inserted in one crate and then each crate processes 512 PMTs. 19 crates tackle 9728 PMT channels, 32 channels are reserved for calibration inputs and labelled as FEC Diagnose (FECD) channels. 

%9605 are actually used
There are , 

s logical channel number (lcn) where
\begin{equation}
