...
The anode pulse produced from the PMT travels along 35 m-long RG59/U type coaxial cable (with a resistance of 75 $\Omega$) to the front-end electronics which are set up on the deck above the detector. The coaxial cable also carries the high-voltage\cite{boger2000sudbury}. To control, manage and take data from over 9000 PMTs in the SNO+ detector, the coaxial cables connected to PMTs are grouped into bundles of eight and the bundles are connected to a Paddle Card. Four Paddle Cards are linked to a PMT Interface Card (PMTIC).
