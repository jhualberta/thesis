...
To tackle with more than 9000 PMTs in the SNO+ detector, the coaxial cables connected to each PMTs are grouped into bundles. Each bundle with 8 PMTs is connected to a Paddle Card. Four Paddle Cards are linked to a PMT Interface Card (PMTIC). The PMTIC supplies high voltages and receives the signals of up to 32 channels. Each channel for one PMT. 32 channels in PMTIC are connected to a Front End Card (FEC) that processes, digitizes and stores PMT signals. Each FEC has 4 Daughter Boards (DB) and each DB handles 8 channels. 16 PMTICs and FECs are inserted in one crate and thus each crate processes 512 PMTs. 19 crates tackle 9728 PMT channels in total, of which 32 channels are reserved for calibration inputs and labelled as FEC Diagnose (FECD) channels. Thus the triggered PMTs can be labelled by the logical channel number (lcn) using the map of the PMT to the crates and cards\cite{snop_nim,stringer2019sensitivity}:
\begin{equation}
lcn = 512 \times crate + 32 \times FEC + channel
\end{equation}
 %9605 are actually used
 
To properly process and record information from PMTs, each DB tackling with 8 channels has three types of custom-made integrated circuits to realize three functions:
\begin{itemize} 
\item[$\bullet$] Two four-channel discriminator (SNOD) chips set thresholds for PMT signals.

Once the PMT pulse crosses the channel threshold, a discrete square trigger pulse is generated. The trigger pulse has different time widths for different trigger types, including a 93 ns long square pulse (N100) and a 48 ns square pulse (N20).
...
\item[$\bullet$] Two eight-channel charge integrator (SNOINT) chips integrate the PMT pulse over time interval to obtain PMT charge information. 

QHS　(charge High gain Short integration time), integrate the pulse over 60 ns

QHL  (Charge High gain Long integration time), integrate the pulse over 400 ns

QLX Charge Low Gain, integrate the low gain PMT pulse and integrate over either time window.





\item[$\bullet$] Eight single-channel CMOS chips to record the time when the PMT pulse cross the thresholds.

time-to-amplitude converter (TAC)

\end{itemize}  

In each crate, the signals from 16 FECs are sent to and summed up by a Crate Trigger Card (CTC). 19 CTCs are connected to 7 Master Trigger Cards/Analogue (MTC/A+) ("+" means an upgrade to SNO system) which sums all the crates

the number of hit PMTs, 

The MTCA checks whether the trigger signals cross the thresholds and then sends the satisfied triggers to the digital master trigger system (MTC/D).

If the total height of either of these summed pulses passes a set threshold value, 

The MTC/D　checks the trigger type with the operator-defined trigger mask, if it is satisfied, then  
a Global Trigger (GT) is issued.


 This global trigger defines an event within the detector and results in all the associated information being saved. Multiple different trigger thresholds can be set. For example: N100Lo, N100Med, N100Hi are triggers based on the N100 pulses, where each has a different threshold value set. So it is possible, and likely, that an event within the detector passes several of these trigger thresholds.


trigger system are to check whether the signal passes through a preset threshold value.
...
of photons across the detector. More specifically, the N100 width is chosen such that a set of photons from a Cherenkov cone will result in overlayed PMT pulses. The shorter N20 width limits this overlaying of PMT pulses to events originating from the centre of the detector.


Additionally, the detector can trigger from the charge in the PMTs directly. The voltage pulses from all of the PMTs are summed together (ESUM). If this combined voltage trace drops below a threshold value (for example, ESUMHi,
...
position on the support structure. A muon event is characterized by the large number of triggered PMTs. As with nearly all muons that pass through the detector, it is traveling in a downward direction. This is seen in Figure 3.4 as the excess of PMT charge (i.e. photons) in the bottom of the detector. As all atmospheric muons travelling upwards would be attenuated by the Earth, the
small flux of upward-going muons are the result of neutrino interactions in the
rock surrounding the detector [56].
Figure 3.5 shows a more typical event. The circular pattern seen is produced by the projection of the Cherenkov light cone onto the PMTs. With 24 triggered
PMTs, this event is considered a high-energy event within the detector. The size of the pattern indicates that this event took place closer to the PMTs travelling in an outward direction, specifically in the water between the acrylic sphere and the PMTs.


Once the global trigger signal is issued, the integrated charges and time from an event are stored in each channel. The values of the TAC, QHS, QHL and QLX (i.e., the time and charge information of triggered PMTs) are read out by a Crate Controller Card (XL3) and sent to the Data Acquisition System (DAQ) and event builder, 
...
\cite{snop_nim,boger2000sudbury,bonventre2014neutron,stringer2019sensitivity}
...
The universal time of an event is calculated as the time elapsed from a predefined $T_{zero}, $the midnight of January 1, 2010 (GMT) to the moment when the event happens．A 10 MHz clock used for counting the absolute time started at $T_{zero}$\cite{rattime}. It has a 53 bit register and can run for 28.5 years. Its accuracy is maintained by a GPS system.

A 50 MHz clock , this clock limits the best time resolution of the GT to 20 ns. This clock has a 43 bit register and rolls over every 2.04 days.



absolute time of that event 


that has  

relative time is the time between events.

PMT hit time refers to all the PMTs triggered by an event.
