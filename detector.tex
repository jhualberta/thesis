...
Tool (\texttt{RAT}), which integrates a Monte Carlo simulation framework for the SNO+ detector and event-based analysis tools (for online and offline event analysis) since the beginning of the program. This software was originally developed by Stan Seibert for Braidwood Collaboration for a generic KamLAND like detector. It is also used by the other astroparticle physics experiments, such as DEAP/CLEAN, CLEAR and potentially for Darkside-50\cite{rat}.

\texttt{Geant4} Toolkit and incorporates ROOT libraries for data handling and analysis.

The \texttt{RAT} Monte Carlo was originally developed for the Braidwood Collaboration
utilizing software developed for a generic KamLAND-like detector, called Generic Liquid Scintillator \texttt{GEANT4} simulation (GLG4). It was branched and
developed into the specialist SNO+ version, starting in 2006/2007.

\texttt{GLG4sim} Braidwood
...
Integrate into existing \texttt{GEANT4} and \texttt{GLG4sim} efforts with a minimum of code duplication. As much as possible, \texttt{RAT} should be designed to incorporate upgrades of these packages just by relinking. No cut and paste of code (mainly a question with GLG4sim).
Design the detailed processes of data acquisition and trigger systems are \cite{whitepaper}.

The SNO+ version of \texttt{RAT} is being developed by the whole collaboration and evolves with the experiment progress.
Besides more functions are added into the \texttt{RAT}, it is also tuned and optimized with the updated parameters from more precise descriptions of the physics processes or the detector responses to the calibration sources. Therefore, different versions of \texttt{RAT} may give different results. For the work in this thesis, multiple \texttt{RAT} versions are used, mainly the versions for the water phase and partial-fill phase. In this case, I will specify the \texttt{RAT} version when I discuss a certain analysis.
\cite{ratManual}