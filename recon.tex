...
For the vertex fitters used in the partial-fill and scintillator phases, since no internal calibration was performed, I adopt a linear interpolation method described in Ref.~\cite{coulter2013modelling}. It will be discussed in Sect.~\ref{sect:scintFitter}.
...
\begin{figure}[htbp]
	...
	\caption[The fit position biases projected on the x, y and z axes.]{The fit position biases projected on the x, y and z axes. The MC generated $10^4$ 5-MeV $e^-$ particles at the detector center with isotropic directions.}
\end{figure}
...
A more generic situation is that the events happen everywhere inside the AV. In this simulation, $e^-$ events were generated uniformly inside the AV volume. The results are shown in Fig.~\ref{fig:MPWposResol_isoFill}.

\begin{figure}[htbp]
	\centering	
	\includegraphics[width=9cm]{MPW_isoFill_posResolVsE.pdf}
	\caption[The \texttt{MPW fitter} position resolutions of x, y, z axes as a function of energies.]{The \texttt{MPW fitter} position resolutions of x (red circle), y (green square), z (black triganle) axes as a function of energies.}
...
\end{equation}
where the parameters: $\beta_M$ and $\beta_S$ are the ``decay'' constants or the ``slopes'' of the two exponential components; $\alpha_M$ is the fraction between two exponential components. The first component, the main peak is due to the single scattering of the electrons and is the true angular resolution of the detector, while the second component which has a broad tail is mainly due to the multiple scattering of electrons; there are also back scattering electrons on the detector components and the poorly reconstructed events in the tails\cite{boulay2004direct}. Fig.~\ref{fig:directionResol_5MeV} shows the $\cos\theta_e$ distribution of the $10^4$ 5-MeV $e^-$ particles generated at the detector center and with isotropic directions.

\begin{figure}[htbp]\label{fig:directionResol_5MeV}
	\centering
	\includegraphics[width=8cm]{MPW_waterDirection_5MeV.pdf}
	\caption[The biases between the fitted and MC directions.]{The biases between the fitted and MC directions, fitted with the resolution function $P(\cos\theta_e)$. The MC generated $10^4$ 5-MeV $e^-$ at the detector center.}
\end{figure}

Another way to quantify the reconstruction performance based on the $\cos\theta_e$ distribution is to calculate the angles that contain 50\%, 80\%and 90\% of the reconstructed events, denoted by $\cos\theta_{0.5}$, $\cos\theta_{0.8}$ and $\cos\theta_{0.9}$ respectively\cite{coulter2013modelling}. Their values were solved numerically by the Eqn.~\ref{eq:cosTheta_e}, taking $\cos\theta_{0.5}$ as an example:
...
With this trigger setting in simulations, about 54.6\% of the low energy 2.2-MeV $\gamma$ events were triggered. So I doubled the simulations to $2\times10^4$.

%Since the low energy 2.2-MeV $\gamma$ has a low chance to trigger the detector ($\sim 8$\%), I simulated $10^5$ $\gamma$ particles to obtain a proper statistics.
...
\begin{figure}[htbp]	
	...
	\caption[Fitted x position.]{Fitted x position. The WLS fitter reconstructed $x$ positions of the 5 MeV electron events in the wbWLS (red) are compared to the ones in the water (blue).
	}
\end{figure}
...
Interestingly, if using a timing $PDF$ with a wrong concentration to the actual data, the fit position biases are small. I simulated 3MeV $e^-$ uniformly distributed in the LAB+0.25 g/L PPO scintillator with isotropic directions, and then reconstructed the simulated events by using the 0.5, 1 and 2 g/L PPO timing $PDF$s respectively. All these reconstructions gave the fitted positions closed to the correct reconstruction using the 0.25 g/L timing $PDF$, with differences in the Gaussian means around 5 mm in x, y and z axes, which were not significant biases. In the other cases, I simulated the same 3MeV $e^-$ events in LAB+2 g/L PPO and reconstructed by using the timing $PDF$s with lower PPO concentrations (0.25, 0.5 and 1 g/L PPO respectively), differences in the Gaussian means were within 5 mm\cite{partialFitterPDFtestInvulnerable}. It shows that the \texttt{MP partial fitter} is invulnerable to the changes caused by different PPO concentrations.
...
5000 simulations of 3MeV $e^-$ generated uniformly inside the AV with isotropic directions. filled with LAB+various concentrations of PPO. 


Partial-fill: 5000 simulations of 3 MeV e- generated uniformly and isotropically in the scintillator region in partial-fill geometry with, with a water level setting at z = 3000 mm in the AV coordination. 
...
\begin{figure}[htbp]\label{fig:scint_groupVelocity}
	\subfigure[Tuning the $v_{gr,scint}$.]{
		...
		\centering
			\includegraphics[width=8cm]{tune_groupVelocity_scint.png}
		\end{minipage}
	}   
	\subfigure[Tuning the $v_{gr,water}$.]{ 
		\begin{minipage}[t]{0.6\textwidth}
			\centering
			\includegraphics[width=8cm]{tune_groupVelocity_water.png}
		\end{minipage}
	}
\end{figure}
...
Simulate 5000 events for 3MeV $e^-$,  $\Delta x = x_{fit}-x_{mc}$
...
Since the 2.5MeV event is the major interested signal in the scintillator and tellurium-loading phases, 10000 2.5MeV $e^-$ events were simulated at random positions inside the AV and with isotropic directions. 
...
\begin{figure}[htbp]
	...
	\caption{Distributions of fit position biases for 10000 MC simulations of 2.5MeV $e^-$ events.}
\end{figure}
