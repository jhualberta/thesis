...
To check the effects of the drive correction, I simulated 5-MeV $e^-$ at the detector center with momentum direction at (1,0,0). It shows that the drive effect in the reconstruction causes a $\sim$+50 mm bias from the detector center in the x axis. The drive correction reduces this drive bias down to $\sim$+0.2 mm in the x axis. The tests on the $^{16}$N central run data show that the drive correction can also improve the 
resolution of the $r_{bias}$ distribution by $\sim$20 mm. In Fig.~\ref{drivecorVsEnergy}, various energies of $e^-$ from 2 to 10 MeV (with a 1-MeV step) were simulated to check the effects before and after the drive correction. These electrons were generated at the detector center and with momentum directions at (1,0,0). The pull is quantified by the radial bias mentioned in Sect.~\ref{sect:tuneGroupVelocity}. The distributions of the $r_{bias}$ in each simulation were fitted with Gaussian functions, and the Gaussian means were used as the pull.
\begin{figure}[!htb]
	\centering
