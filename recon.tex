...
Since the parameters of the drive correction were obtained from the reconstructions of the MC simulations, it depends on the simulations. Therefore, the n$_{water}$, mode cut and time residual cut affecting the results of reconstruction will also affect the drive correction parameters, but not significantly.
...
To check the effects of the drive correction, I simulated 5-MeV $e^-$ at the detector center with (along +x axis)


By fitting the simulations of and traveling along the +x direction, 



the drive effect of the \texttt{MPW fitter} causes a $\sim$50 mm biases from the detector center along the +x axis. The drive correction reduces this drive bias down to $\sim$0.2 mm. For the reconstruction of $^{16}$N data, the drive correction can reduce the fitted position RMS by $\sim$20 mm. In Fig.~\ref{drivecorVsEnergy}, various energies of $e^-$ from 2 to 10 MeV were simulated to check the effects before and after the drive correction. These electrons were generated at the detector center and with momentum directions of (1,0,0). The pull is quantified by radial bias mentioned in Sect.~\ref{sect:tuneGroupVelocity}. The mean value of the distributions of the $r_{bias}$ in each simulation is used as the pull in the y-axis. 
\begin{figure}[!htb]
	\centering
	\includegraphics[width=8cm]{pullEffectVsEnergy.pdf}
	\caption[Radial biases of the simulated $e^-$ events as a function of energy.]{Radial biases of the simulated $e^-$ events before and after the drive correction, as a function of energy. The results from the official \texttt{RAT water fitter} are also shown here.}
...
A quantity called scaled $\log L$ ($scaleLogL$) is used as the position reconstruction FoM ($posFoM$): $scaleLogL = \ln L/\mathrm{NHit}_{selected}$. This quantity utilizes the best log-likelihood returned by the \texttt{MP fitter} for a successfully reconstructed event vertex, and then it is scaled by the ``selected'' NHit ($\mathrm{NHit}_{selected}$), which is the number of the PMTs actually used by the fitter for the event vertex reconstruction, after the PMT selections mentioned in Sect.~\ref{sect:PMTselector}. This $posFoM$ quantity can remove some mis-reconstruct events and then improve the results of the reconstruction.
...
A cut of $scaleLogL>10$ can remove a few mis-reconstruction . 
...
Following the same method of tuning the $v_{gr,eff}$ in Sect.~\ref{sect:tuneGroupVelocity}, 
...
\begin{figure}[htbp]\label{fig:partial_scint}
	...
	\subfigure[$x_{fit}-x_{mc}$.]{
		\begin{minipage}[t]{0.38\textwidth}\label{fig:partial_scint_b}
			\includegraphics[width=6.5cm]{partial_top_3MeV_xbias.pdf}
		\end{minipage}
	}   
	\subfigure[$y_{fit}-y_{mc}$.]{ 
		\begin{minipage}[t]{0.38\textwidth}\label{fig:partial_scint_c}
			\centering
			\includegraphics[width=6.5cm]{partial_top_3MeV_ybias.pdf}
		\end{minipage}
	}
	\subfigure[$z_{fit}-z_{mc}$.]{ 
		\begin{minipage}[b]{0.38\textwidth}\label{fig:partial_scint_d}
			\centering
