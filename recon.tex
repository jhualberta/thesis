...
Up till this thesis writing, there is no proper energy fitter for the partial-fill phase. I attempted two methods for energy reconstruction in the partial-fill phase: the NHits scale method, based on Ref.~\ref{partialEnergy} and the NHits ratio method. Both of them are based on simulations of 

in different detector positions. 

These two methods need more effects to be improved.
...
%The previous partialEnergy fitter in RAT was based on Nhits scaling with water level and event z position (docDB-3133). However, for the events below the water level, the Nhits scaling is ill-defined and this causes bias in results, see docDB-5727. This energy fitter currenlty is turned off in the partialFitter.
%Revisit the charge(Q)-ratio fitter developed by Yang Zhang (docDB-5543-v2).
%    - This fitter checks the sum of the QHS values of an event (total charge, Q) and then fit to the energy by a third-order polynomial function E' = fE(Q).
%    - It also corrects the Q with the position (ρ,z), where ρ=sqrt(x2+y2). This position dependence is considered as geometry or systematic effects, not for scaling/weighting the charges of the events in scintillator or water regions.
%    - It calculates the charge ratios (r) of the other positions to a reference point (choose the point(1000,-1000,-1000) mm)  in the full scintillator geometry (or the water level, split_z = -6000 mm). Then Q' = Q/r.
%    - It then corrects the Q' according to the water level by a third-order polynomial, Q'' = fQ'(split_z).
%    - Finally, Efit = fE(Q'').
%    - Assume chan_eff: 0.75058, active_channels: 9394.
%
%Tune and optimize the Q-partial fitter to Rat-6.18.0, for 0.5 g/L PPO concentration and water level at 747.5 mm.
% Not consider the dependence of water level (no split_z correction) since we are focusing on the data at 747.5 mm analysis currently.
%Here the Q-ratio table is for scaling/weighting the events in scintillator/water (not the same to the original one): 
%Create the Q-ratio table:
%Generate 2.5 MeV e- at different position points with any directions, in the partial-fill geometry. 
%Assume a symmetry in x-y plane, keep y = 0 and move x from 0 to 5500 mm and z from -5500 to 5500  mm by a step of 500 mm.  Only the (ρ,z) bins inside the AV is considered (we can even set only the inner_av_top region is valid for energy reconstruction). Total bin is 12*23 = 276.
%For each position point, there are 2000 simulations and the total charge Q is the sum of QHS values for each triggered event, averaged by all the triggered events.
%Here the total charge of the simulations of 2.5 MeV e- uniformly distributed in the scintillator region (inner_av_top) is used as the reference charge. In this case, the ratios for the events in the inner_av_top can be considered as corrections for systematics while the ratios of the water events are the scaling values (events with the same energy create ~32 times less nhits than the ones in the scintillator).
%
%
%1 and 3 MeV e- in the scintillator region with any directions, 0.5 g/L PPO, split_z = 747.5 mm, 1000 simulations. 
