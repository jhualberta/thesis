...
\end{equation}
where $t_{PMT}$ is the PMT triggered time recorded by the detector, $t_{event}$ is the time when an event occurs, and $t_{transit}$ is the total transit time (or time of flight, $TOF$) taken by a photon travelling from the event position ($\vec{x}_{event}$) to the triggered PMT ($\vec{x}_{PMT}$) and crossing different materials in the detector.
