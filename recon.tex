...
\begin{figure}
	...
	\caption{Evaluate the light path in the scintillator region.}\label{lightpath_scintAV}
\end{figure}


Then combine the evaluations of the ray-sphere and the ray-cylinder intersections to calculate the light path in the neck scintillator region ($d_{sp,neck}$):
...
\hspace{2mm}if $a'_+a'_-<0$ (event position is inside the cylinder), check the z position of the intersection point on neck, $z_+ = z_0 + a'_+u_z$: 
	
\hspace{6mm}if $6108<z_+<8390~mm$ (in the valid neck region), then check the AV sphere:
	 
\hspace{8mm}if $|\vec{X}_0|\geq r_{AV}$, $d_{sp,neck}=a'_+$, see Fig.~\ref{lightpath_scintNeck} (a).
	
\hspace{8mm}if $|\vec{X}_0|<r_{AV}$ and $a_+a_-<0$:
	
\hspace{10mm}the light ray first hits the sphere inside the cylinder and then hits the cylinder, then $d_{sp,neck}=a'_+-a_+$, see Fig.~\ref{lightpath_scintNeck} (b). 
	
\hspace{6mm}if $z_+<6108~mm$:
	
\hspace{8mm}if $|\vec X_0|\geq r_{AV}$ and $6108<z_0<8390~mm$:
	
\hspace{10mm}if $a_+>a_->0$: $d_{sp,neck}=a_-$, see Fig.~\ref{lightpath_scintNeck} (c).
			
\hspace{2mm}if $a'_+>a'_->0$ (event position is outside the cylinder), check the z position of the intersection point on neck, $z'_{\pm}=z_0+a'_\pm\cdot u_z$:
	 
\hspace{4mm}if $6108<z_\pm<8390~mm$: 
	 
\hspace{6mm}$d_{sp,neck}=a'_+ - a'_-$, see Fig.~\ref{lightpath_scintNeck} (d).
	 
\hspace{6mm} if $a_+>a_->0$:
	 
\hspace{8mm} Evaluate the z positions of the ray-sphere intersection points: $z_\pm=z_0+a_\pm\cdot u_z$
	  
\hspace{10mm}if $z_\pm\geq 6108~mm$: $d_{sp,neck}=a'_+ - a'_--(a_+-a_-)$, see Fig.~\ref{lightpath_scintNeck} (e). The path inside the sphere is subtracted to avoid duplicated calculation.
	 
\hspace{10mm}if $z_+<6108$ and $6108<z_-<8390$ (if one intersection point below neck while the other in neck):
	 
\hspace{12mm}if $a_+>a_->0$: $d_{sp,neck}=a_--a'_-$, see Fig.~\ref{lightpath_scintNeck} (f).
			   
\hspace{12mm}if $z_-<6108$ and $6108<z_+<8390~mm$:
               
\hspace{14mm}in this case, either the event position is inside the sphere ($a_+a_-<0$, see Fig.~\ref{lightpath_scintNeck} (g)), or outside the sphere ($a_+a_-<0$, see Fig.~\ref{lightpath_scintNeck} (h)), the path in neck is the same: $d_{sp,neck}=a'_+ - a_+$.
\end{algorithm}
