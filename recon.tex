...
\section{An Overview for the Reconstruction Algorithms in SNO+}
An event happens in the SNO+ detector can produce Cherenkov photons or scintillation photons and these photons propagate through the detector and finally reach the PMTs and are detected.


to determine the vertex (position and time), direction and energy of an event.
...
\section{Multi-path Vertex Reconstruction Algorithm for SNO+}\label{water_vertex}
...
\section{Effective Group Velocity}\label{tuneGroupVelocity}
...
\begin{figure}[!htb]
	...
	\caption{ A cartoon illustrates the drive correction.}
	\label{drivecor}
\end{figure}
...
This s a fit method class modified from ROOT TDecompQRH class\cite{TDecompQRH}. It is used by the Multi-path fitter to invert the Hessian matrix through QR decomposition method\cite{press2007numerica}. Compared to ROOT, Solve() for Ax=b is modified to zero the component of x for which the diagonal element in R is small. This allows a Levenberg-Marquardt optimization to continue in many cases
...
Detailed descriptions are shown in section \ref{PMTselector}.

\end{itemize}

\section{Vertex and Direction Reconstruction for Water-based Wavelength-shifter}
...
\section{Vertex Reconstruction for Partial-fill and Scintillator Phases}\label{scintFitter}
...
For a trial position $\vec{X}_0=(x_0,y_0,z_0)$ and a triggered PMT position $\vec{X}_{\mathrm{pmt}}=(x_\mathrm{pmt},y_\mathrm{pmt},z_\mathrm{pmt})$, define a ray-vector (rather than a line without direction) $\vec{l}_0\equiv\vec{X}_0+a\cdot \vec{u}$, where $a$ is the distance between vertex and intersection point and it is the parameter to be determined; $\vec u=(\vec{X}_{\mathrm{pmt}}-\vec{X}_0)$ is the direction of the ray-vector or light path. It is a unit vector pointing from the $\vec{X}_0$ to the $\vec{X}_{\mathrm{pmt}}$.

In the ray-sphere intersection case (light path passes through the AV sphere), the intersection equation is:
$(\vec{l}_0-\vec{O}_{av})^2 = r^2_{av}$, where $\vec{O}_{av}$ is the origin of the AV sphere. In the PSUP coordinate, $\vec{O}_{av} = (0,0,108)~mm$.
...
In this case, both $a_+$ and $a_-$ exist and have different values. If $a_+>a_->0$, the length of the path inside the sphere is $a_+-a_-$, as illustrated in Fig.~\ref{line-sphere} (a). Due to this geometry, the event position should be outside the AV, the condition $|\vec{X}_0|\geq r_{AV}$ is automatically met. If $a_+>0>a_-$, $a_-$ determines an intersection point along the opposite direction of the ray-vector, which means the ray-vector actually does not pass that point (though the line without direction passes). Thus the length of the path inside the sphere is $a_+$, as illustrated in Fig.~\ref{line-sphere} (b). Also, the condition $|\vec{X}_0|<r_{AV}$ is automatically met. 

If $\Delta\leq0$,
there is no intersection point or only one intersection point at the AV, the light path never passes through the AV sphere, as illustrated in Fig.~\ref{line-sphere} (c) and (d).
...
For the ray-plane intersection, the equation is:
$l_{0,z}=Z_{split}$, where $Z_{split}$ is the water level, i.e., the z position of the water-scintillator intersection and $l_{0,z}=z_0+a\cdot u_z$.
...
\]
Similar to the case of ray-sphere intersection, if $a_3<0$, the ray-plane intersection point is on the extended line along the opposite direction to the ray; $a_3 \geq 0$ ensures the ray hits the interface. If $a_3 = 0$, 

For the ray-cylinder intersection, the equation is:
$l^2_{0,x}+l^2_{0,y} = r^2_{neck}$, where $ r_{neck}$ is the radius of the neck cylinder ($r_{neck}=785~mm$).
Solve the equation we get: 


$\Delta = bneck*bneck - ( fx*fx+fy*fy-r^2_{neck})*drPerp2$

\[
a_{neck,\pm} = |\vec{X}_{PMT}-\vec{X}_0|\cdot\frac{-bneck \pm \sqrt\Delta }{(x_{PMT}-x_0)^2+(y_{PMT}-y_0)^2},~if~\Delta>0,
\]
...
To evaluate the length of the light path in the scintillator region ($d_{sp}$), the above three geometries needs to be combined carefully. The following procedure goes through all the possible situations. First combine the evaluations of the ray-sphere and the ray-plane intersections to calculate the length of the light path in the AV scintillator region ($d_{sp,AV}$):

\begin{algorithm}
First check the ray-sphere intersection:\\
If {$\Delta>0$}, 

\hspace{2mm}if $|\vec{x}_0|<r_{AV}$ (and $a_+>0>a_-$), 

\hspace{2mm}check the ray-plane intersection:

\hspace{4mm}if $a_3>0$, the ray-vector hits the interface plane: 

\hspace{6mm}if $z_0<Z_{split}$ and $a_3<a_+$:

\hspace{8mm}$d_{sp,AV}=a_+-a_3$, see Fig.~\ref{lightpath_scintAV} (a).

\hspace{6mm}if $z_0\geq Z_{split}$:

\hspace{8mm}if $a_3<a_+$:  $d_{sp,AV}=a_3$, see Fig.~\ref{lightpath_scintAV} (b). 

\hspace{8mm}if $a_3\geq a_+$:  $d_{sp,AV}=a_+$, see Fig.~\ref{lightpath_scintAV} (c).

\hspace{4mm}if $a_3\leq0$ and $z_0>Z_{split}$:

\hspace{6mm}$d_{sp,AV}=a_+$, see Fig.~\ref{lightpath_scintAV} (d).

\hspace{2mm}if $|\vec{x}_0|\geq r_{AV}$ (and $a_+>a_->0$), calculate the z position of the intersection point: 

\hspace{4mm}$z_{\pm}=z_0+a_\pm\cdot(z_{PMT}-z_0)/|\vec{X}_{PMT}-\vec{X}_0|$,

\hspace{4mm}if $z_- \geq Z_{split}$ and $z_+\geq Z_{split}$:

\hspace{8mm}$d_{sp,AV} = a_+ - a_-$, see Fig.~\ref{lightpath_scintAV} (e).

\hspace{6mm}if $z_- \geq Z_{split}$ and $z_+\geq Z_{split}$ and $a_3>0$:
	
\hspace{8mm}if $z_-< Z_{split}$ and $z_+> Z_{split}$:

\hspace{10mm}$d_{sp,AV} = a_+ - a_3$, see Fig.~\ref{lightpath_scintAV} (f).

\hspace{8mm}if $z_->Z_{split}$ and $z_+<Z_{split}$:

\hspace{10mm}$d_{sp,AV}= a_3 - a_-$, see Fig.~\ref{lightpath_scintAV} (g).
\end{algorithm}
	
	
\begin{figure}
	\centering
	{\includegraphics[width=100mm]{scintPathAV.png}}
	\caption{Evaluate the length of the light path in the scintillator region.}\label{lightpath_scintAV}
\end{figure}


Then combine the evaluations of the ray-sphere and the ray-cylinder intersections to calculate the length of the light path in the neck scintillator region ($d_{sp,neck}$):

\begin{figure}
	\centering
	{\includegraphics[width=80mm]{sphere-plane1.png}}
	\caption{Evaluate the length of the light path in the neck region.}\label{lightpath_scintNeck}
\end{figure}



Since the valid fit requires the events inside the PSUP sphere, only the neck region inside the PSUP (with $6108<z_{neck}<8390~mm$) needs to be considered. the neck path calculation is allowed to be turned off.
...
Once the lengths of the light path in the scintillator region or the water region are calculated, the time of flight, $t_{transit}$ is obtained by:
\begin{equation}
...
amplitude $a_i$ are determined by the benchtop measurements \cite{}.
...
\begin{table}[ht]
	\caption[]{\label{scint_timing} scintillator $\alpha/\beta$ timing parameters\cite{tanner0p5,joshW1,chicagoTiming}.}		
	{\centering
...
\section{PMT Selectors for the Fitter}\label{PMTselector}
